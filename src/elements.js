...
/* [отрисовка, поле, стиль] → предел в знаках, либо null, если поле в этом
   стиле подписью не рисуется (у retro дельта kpi — шкала из блоков, у aurora
   плашки нет вовсе). Двоичный поиск: порядок монотонен, и это не допущение —
   проба в tools/data-check.js проверяет его сплошным перебором. */
function пределЗнаков(отрисовка, поле, s) {
  if (typeof отрисовка !== 'function' || отрисовка.length < 2 || !s) return null;
  const пам = ячейка(ячейка(памятьПределов, отрисовка), s);
  if (пам.has(поле)) return пам.get(поле);
  const влезает = n => {
    const проба = ПРОБА_ПРЕДЕЛА.slice(0, n);
    let svg; try { svg = отрисовка(s, { [поле]: проба }, []); } catch { return false; }
    return доехала(svg, проба);
  };
  let итог = null;
  if (влезает(1)) {
...
export { CATS, пределЗнаков, словами };
