...
/* ══════ МЕРА ТЕКСТА ══════
   Ширина строки в пикселях. Точной её не сделать, и это не лень: разметка
   объявляет font-family="Inter,system-ui,sans-serif", а Inter — веб-шрифт,
   которого штатно нет ни в Windows, ни в macOS (проверено: C:/Windows/Fonts
   даёт arial, segoeui, tahoma, файлов Inter — ноль). Значит почти везде
   рисуется запасной: Segoe UI либо SF Pro. Раз шрифт зрителя неизвестен,
   мера обязана ЗАВЫШАТЬ: тогда текст ужмётся раньше времени, но не вылезет.

   Отсюда четыре класса и метрики Arial — он шире Segoe UI из двух:

     прописная (кириллица и латиница)  0.73    Arial 0.726, Segoe UI 0.684
     цифры                             0.556
     пробел                            0.28
     всё остальное                     0.55    строчная 0.578, по частоте 0.548

   Прежняя единая 0.55 хорошо ложилась на строчную кириллицу и врала
   на прописной впятую: «ВЫРУЧКА» оценивалась в 0.81 настоящей ширины.

   Два признака мера получает от места вызова, потому что из самой строки
   их не видно:
     s      — стиль. Семь стилей из тринадцати поднимают регистр, и делает
              это t() уже ПОСЛЕ замера (engine.js:283): мера смотрела
              на строчные буквы, а холст получал прописные.
     зазор  — letter-spacing. SVG добавляет его после каждого знака, включая
              последний, а в семи стилях с капсом он 1.4–2.4px: при кегле 13
              это ещё треть ширины сверху.
   Отрицательный letter-spacing (в библиотеке есть, до -2) мера нарочно
   не учитывает: занижать оценку нельзя, завышать можно. */
const ШИР = { прописная: 0.73, цифра: 0.556, пробел: 0.28, прочее: 0.55 };   // spec/tokens.json
const классЗнака = c => /[A-ZА-ЯЁ]/.test(c) ? ШИР.прописная
  : /[0-9]/.test(c) ? ШИР.цифра : c === ' ' ? ШИР.пробел : ШИР.прочее;
const мера = (str, fs, оп) => {
  const т = оп && оп.s && оп.s.caps ? String(str).toUpperCase() : String(str);
  const зз = оп && оп.зазор > 0 ? оп.зазор : 0;
  return [...т].reduce((ш, c) => ш + классЗнака(c), 0) * fs + зз * т.length;
};

/* Признаки приходят одним именованным объектом {сбор, слот, s, зазор},
   а не двумя доводами в хвост. Причина не в красоте: седьмой и восьмой
   позиционные молча пропускались бы там, где нужен только один из них,
   а шесть позиционных — уже предел читаемости. Именованный объект вдобавок
   ловит недоделку громко: если где-то останется прежний вызов с «сбор»
   пятым доводом, сюда придёт массив, и страж ниже уронит отрисовку
   на первом же прогоне вместо того, чтобы тихо потерять признаки. */
const признаки = оп => {
  if (оп === undefined || оп === null) return undefined;
  if (typeof оп !== 'object' || Array.isArray(оп))
    throw new TypeError('подгонка: пятый довод — объект признаков {сбор, слот, s, зазор}, а не сбор');
  return оп;
};


/* ══════ КАНАЛ ЗАМЕЧАНИЙ О ДАННЫХ ══════
   Между сверитьДанные (состав полей, src/index.js) и шестью проверками
   (готовая разметка, src/checks.js) была пустота: элемент видит беду
   в самих значениях — итог не сходится с накоплением, накопление ушло
   в минус, значение зажато по границам договора, переданное поле композиция
   не рисует, — а сказать об этом ему было нечем.

   Способ выбран третьим аргументом-сборщиком: массив, в который функция
   отрисовки складывает записи. Второй возвращаемый элемент переписал бы
   полсотни вызовов; поле на самой функции не переживает параллельных
   вызовов, а сервер у нас один на всех агентов. Сборщик не ломает никого:
   вызов в два аргумента и вызов вовсе без данных остаются как были,
   а Array.isArray отсеивает и undefined, и чужое.

   Запись машиночитаема нарочно. Наш потребитель — модель-сборщик, и ей
   «steps[2].v: передано 999, накоплено 120» — прямое указание, что чинить.
   Замечание никогда не отменяет отрисовку: элемент рисует всегда. */
const весть = (сбор, запись) => { if (Array.isArray(сбор)) сбор.push(запись); return запись; };

/* Зажатие по договорным границам. Арифметика прежняя, до знака: меняется
   только то, что о подмене теперь говорят. «Значение 120 зажато до 100
   по границам 0–100» полезно, «данные подправлены» бесполезно. */
const вРамках = (сбор, слот, исх, низ, верх) => {
  const стало = Math.max(низ, Math.min(верх, исх));
  /* Молчать надо ровно об одном — о не-числе: NaN !== NaN, и без стража
     всякое нечисловое поле давало бы «зажато NaN до NaN». Про такое поле
     уже говорит сверитьДанные («ждёт число, пришло string»), второго голоса
     не нужно. Стражем стояло Number.isFinite — и заодно глушило ±Infinity,
     а это самое настоящее зажатие: значение подменено конечным числом,
     и никто об этом не узнавал. Случай достижим обычным JSON —
     JSON.parse('1e400') даёт Infinity. Теперь страж отсекает только NaN. */
  if (стало !== исх && !Number.isNaN(исх))
    весть(сбор, { 'вид': 'зажато', 'слот': слот, 'было': поJSON(исх), 'стало': стало, 'границы': [низ, верх] });
  return стало;
};

/* Ответ уходит через JSON.stringify, а он превращает ±Infinity в null:
   запись «было: null» не сказала бы модели ничего и вдобавок соврала бы
   пустотой. Отдаём такое значение строкой — «Infinity» переживает
   сериализацию и читается так же однозначно, как число. Конечные числа
   остаются числами, тип поля меняется только там, где числа уже нет. */
const поJSON = ч => Number.isFinite(ч) ? ч : String(ч);

/* Запись словами — для «предупреждений» ответа draw. Машиночитаемая запись
   уходит в ответ отдельным полем; строка её не заменяет, а проговаривает. */
function словами(з) {
  switch (з['вид']) {
    case 'зажато':
      return `${з['слот']}: значение ${з['было']} зажато до ${з['стало']} по границам договора ` +
        `${з['границы'][0]}–${з['границы'][1]}`;
    case 'не сходится':
      return `${з['слот']}: передан итог ${з['заявлено']}, а накопление предыдущих шагов даёт ` +
        `${з['вычислено']} — нарисовано накопленное, картинка не врёт арифметикой`;
    case 'ниже оси':
      return `${з['слот']}: накопление ушло в минус (${з['накоплено']}) — столбец встаёт ` +
        'не по ту сторону оси, водопад читается неверно';
    case 'вне договора': {
      const что = з['было'] === null ? 'поле не передано' : `передано «${з['было']}»`;
      return `${з['слот']}: ${что} — ${з['почему']}` +
        (з['стало'] !== з['было'] ? `; принято «${з['стало']}»` : '');
    }
    case 'взято из образца':
      return `${з['слот']}: ${з['почему']} — «${з['нарисовано']}»`;
    case 'не нарисовано':
      return `${з['слот']}: передано значений ${з['сколько']}, нарисовано ${з['нарисовано']} — ${з['почему']}`;
    case 'не влезло':
      return `${з['слот']}: «${з['текст']}» требует ${з['требуется']}px при доступных ${з['доступно']}px — ${з['сделано']}`;
    default:
...
/* ══════ ПОЛ КЕГЛЯ ══════
   Решение владельца, спрошено дважды и с таблицей цены: 9. На половине
   слайда это 14.4pt — нижняя граница, при которой подпись ещё читается
   в зале. Ниже ужимать нечего: нечитаемая подпись обесценивает элемент
   целиком, поэтому ниже пола подпись меняет стратегию — обрезается
   многоточием. Пол в отрасли есть ровно у двоих: MSO — 25% исходного
   кегля, tldraw — 14px абсолютных (measureNoteLabelSize, явное
   «if (fontSizeAdjustment <= 14) break»). Наши 25% нереальны: при
   подписях 8–13px на холсте 300×190 это два-три пикселя.

   Раньше пол был свой у каждого места вызова — minFs || fs × 0.72, —
   и у поля с кеглем 9 равнялся 6.48, то есть подпись падала до шести.

   Math.min(fs, ПОЛ_КЕГЛЯ) — НЕ описка и не перестраховка. У одиннадцати
   полей договора объявленный кегль сам ниже девяти (7.5 у egov.note
   и layers.note, 8–8.5 ещё у девяти). Единый пол 9 встал бы ВЫШЕ их
   собственного кегля, и «не ниже 9» означало бы для них «не ниже своего
   кегля». Это ровно то же самое, но прочитанное как ошибка, — пишем явно:
   пол поля есть меньшее из его кегля и общего пола.

   Math.max(...) — тоже по делу: пол ПОДНИМАЕТ планку, а не заменяет её.
   Место вызова вправе требовать больше (у donut.center планка 13), и пол
   не смеет опустить такое требование до девяти. */
const ПОЛ_КЕГЛЯ = 9;                 // spec/rules.json → текст.подгонка_по_ширине.пол_кегля
const полКегля = (fs, minFs) => Math.max(minFs || fs * 0.72, Math.min(fs, ПОЛ_КЕГЛЯ));

/* Одна строка в заданную ширину: [текст, кегль] */
function ужать(str, maxW, fs, minFs, оп) {
  оп = признаки(оп);
  str = String(str ?? ''); minFs = полКегля(fs, minFs);
  let f = fs;
  while (мера(str, f, оп) > maxW && f > minFs) f -= 0.5;
  if (мера(str, f, оп) <= maxW) return [str, f];
  let о = str;
  while (о.length > 1 && мера(о + '…', f, оп) > maxW) о = о.slice(0, -1);
  переполнение(оп, str, мера(str, f, оп), maxW, 'обрезано');
  return [о + '…', f];
}

/* Отчёт о переполнении — машиночитаемый, и в этом всё дело. Потолок отрасли
   (разведка 22 августа) — строчка в stderr у Graphviz при fixedsize=true;
   Vega и ECharts режут молча, python-pptx возвращает None без исключения.
   Человеку строчка бесполезна, а модели-сборщику «levels[1].label требует
   96.4 при доступных 78.0» — готовое указание сократить текст и позвать
   draw заново. Поле аддитивно: SVG не меняется, слепки остаются валидными. */
const переполнение = (оп, текст, требуется, доступно, сделано) => {
  if (!оп || !Array.isArray(оп.сбор) || !оп.слот) return;
  весть(оп.сбор, { 'вид': 'не влезло', 'слот': оп.слот, 'текст': String(текст),
    'требуется': +Number(требуется).toFixed(1), 'доступно': +Number(доступно).toFixed(1), 'сделано': сделано });
};
...
function ужатьБезТочек(str, maxW, fs, minFs, оп) {
  оп = признаки(оп);
  str = String(str ?? ''); minFs = полКегля(fs, minFs);
  let f = fs;
  while (мера(str, f, оп) > maxW && f > minFs) f -= 0.5;
  ...
  let о = str;
  while (о.length > 1 && мера(о, f, оп) > maxW) о = о.slice(0, -1);
  переполнение(оп, str, мера(str, f, оп), maxW, 'обрезано без многоточия');
  return [о, f];
}

/* Общий кегль на набор строк — чтобы подписи в ряду не разъезжались по размеру */
/* Первый проход ищет общий кегль и замечаний не даёт — иначе про каждую
   строку набора сказали бы дважды. Второй раздаёт слоты по номеру строки. */
const безЗаписи = оп => оп && { s: оп.s, зазор: оп.зазор };
const дляРяда = (оп, i) => оп && { сбор: оп.сбор, слот: оп.слоты && оп.слоты[i], s: оп.s, зазор: оп.зазор };
function ужатьНабор(список, maxW, fs, minFs, оп) {
  оп = признаки(оп);
  const б = безЗаписи(оп);
  const f = список.reduce((м, с) => Math.min(м, ужать(с, maxW, fs, minFs, б)[1]), fs);
  return [список.map((с, i) => ужать(с, maxW, f, f, дляРяда(оп, i))[0]), f];
}

function ужатьНаборБезТочек(список, maxW, fs, minFs, оп) {
  оп = признаки(оп);
  const б = безЗаписи(оп);
  const f = список.reduce((м, с) => Math.min(м, ужатьБезТочек(с, maxW, fs, minFs, б)[1]), fs);
  return [список.map((с, i) => ужатьБезТочек(с, maxW, f, f, дляРяда(оп, i))[0]), f];
}
...
function elKpi(s,дан,зам){
  ...
  const [лбл,лfs]=ужать(label,bw,13,null,{s,зазор:s.ls});
...
function elBars(s,дан,зам){
  ...
  if(заг){const [згТ,згFs]=ужать(заг,bw,12,9,{сбор:зам,слот:'title',s,зазор:s.ls});o+=txt(s,cx,y+24,t(s,згТ),згFs,s.wl,s.ts,s.ls);}
  // подпись слева не должна доехать до значения справа
  const [подписи,fs]=ужатьНабор(rows.map(р=>t(s,String(р.label==null?'':р.label))),bw-44,12,8.5,{сбор:зам,слоты:rows.map((_,i)=>`rows[${i}].label`),s,зазор:s.ls});
  rows.forEach((р,i)=>{
    const v=вРамках(зам,`rows[${i}].v`,Number(р.v)||0,0,100),c=пал[i%пал.length],by=top+i*шаг;
...
function elColumn(s,дан,зам){
  ...
  const [метки,мfs]=ужатьНабор(ряд.map(б=>t(s,String(б.label==null?'':б.label))),bw,11,7,{s});
  ряд.forEach((б,i)=>{
...
function elDonut(s,дан,зам){
  ...
  const [имена,лfs]=ужатьНабор(легенда.map(п=>t(s,п[0])),x+w-20-(lx+15),11,8.5,{s});
  легенда.forEach((п,i)=>{
...
function elRadar(s,дан,зам){
  ...
  const vals=оси.map((о,i)=>вРамках(зам,`axes[${i}].v`,Number(о.v)||0,0,100));
  const labs=оси.map(о=>String(о.label==null?'':о.label));
  const заг=строка(дан&&дан.title,ОБР_radar.title);
  const [згТ,згFs]=ужать(заг,w-40,12,9,{сбор:зам,слот:'title',s,зазор:s.ls});
...
function elGauge(s,дан,зам){
  ...
  const V=вРамках(зам,'value',дан&&дан.value!=null&&!Number.isNaN(Number(дан.value))?Number(дан.value):ОБР_gauge.value,0,100);
  ...
  const [пдТ,пдFs]=ужать(подпись,w-40,10,null,{s});
...
function elArea(s,дан,зам){
  ...
  const [метки,мfs]=ужатьНабор(лбл.map(л=>t(s,л)),pw/Math.max(1,к)-6,9,6.5,{s});
  лбл.forEach((л,i)=>
...
function elWaterfall(s,дан,зам){
  const x=PAD,y=PAD,w=W-PAD*2,h=H-PAD*2,iso=s.mode==='iso';
  let o=frame(s)+surface(s,x,y,iso?w-s.dx:w,iso?h+s.dy:h);
  ...
  // Нормализация осталась прежней — молчание при ней кончилось: договор
  // требует t у первого и последнего шага (база и итог), и подмена типа,
  // как и нарушение этого требования, уходит замечанием
  const обр=шагиДан.map((ш,i)=>{
    const тип=['t','p','n'].includes(ш.type)?ш.type:'p';
    /* Непереданное поле и переданная пустая строка — разные беды, и модели
       от них нужно разное: одну дописать, другую исправить. null означает
       «поля не было», строка — то, что пришло, хотя бы и пустое */
    const исходный=ш.type==null?null:String(ш.type);
    if(тип!==ш.type)
      весть(зам,{'вид':'вне договора','слот':`steps[${i}].type`,'было':исходный,'стало':тип,
        'почему':'договор знает только t (база или итог), p (приращение) и n (потеря)'});
    if((i===0||i===N2-1)&&тип!=='t')
      весть(зам,{'вид':'вне договора','слот':`steps[${i}].type`,'было':исходный,'стало':тип,
        'почему':i===0?'первый шаг обязан быть типа t: это база':'последний шаг обязан быть типа t: это итог'});
    return {
  ...
  /* Накопление считается один раз и используется дважды: сперва чтобы вывести
     ...
     который врёт арифметикой. Выбор в пользу верной арифметики остался прежним,
     а вот молчание кончилось: расхождение уходит замечанием, и модель-сборщик
     узнаёт, что нарисовано не то число, которое она передала.

     Отдельно — накопление, ушедшее в минус: столбец падающего шага рисуется
     как сумма модулей, и по ту сторону оси картинка перестаёт быть водопадом.
     Дефект предсуществующий, раскладку он здесь не меняет — но и молчать
     о нём больше нечего ради */
  let накоп=0;
  let ушлоВМинус=false;
  const шаги=обр.map((ш,i)=>{
    let v2=ш.v;
    if(ш.type==='t'&&i===N2-1){
      if(ш.v!==накоп)
        весть(зам,{'вид':'не сходится','слот':`steps[${i}].v`,'заявлено':ш.v,'вычислено':накоп,
          'сделано':'нарисовано накопленное'});
      v2=накоп;
    }
    if(ш.type==='t') накоп=v2; else накоп+=v2;
    if(накоп<0&&!ушлоВМинус){
      ушлоВМинус=true;
      весть(зам,{'вид':'ниже оси','слот':`steps[${i}]`,'накоплено':накоп});
    }
...
function elHeat(s,дан,зам){
  ...
  const M=(Array.isArray(дан&&дан.matrix)&&дан.matrix.length?дан.matrix:ОБР_heat.matrix)
    .slice(0,5).map((р,ri)=>(Array.isArray(р)?р:[]).slice(0,6).map((v,ci)=>вРамках(зам,`matrix[${ri}][${ci}]`,Number(v)||0,1,9)));
  ...
  const [days,daysFs]=ужатьНабор(ряды(дан&&дан.cols,ОБР_heat.cols,нc).map(String),cw,9,7,{сбор:зам,слоты:ряды(дан&&дан.cols,ОБР_heat.cols,нc).map((_,i)=>`cols[${i}]`),s});
  ...
  const [wks,wksFs]=ужатьНаборБезТочек(ряды(дан&&дан.rows,ОБР_heat.rows,нr).map(String),ox-x-24,9,7,{сбор:зам,слоты:ряды(дан&&дан.rows,ОБР_heat.rows,нr).map((_,i)=>`rows[${i}]`)});
...
function elTree(s,дан,зам){
  ...
  B.forEach((b,i)=>{
    ...
    if(bw>52){
      const [блТ,блFs]=ужать(t(s,b.l),bw-14,9.5,7,{s});
...
function elScatter(s,дан,зам){
  ...
  const P=ряды(дан&&дан.points,ОБР_scat.points,16)
    ...
    .map(([a,b,hi],i)=>[вРамках(зам,`points[${i}].x`,a,0,100),вРамках(зам,`points[${i}].y`,b,0,100),hi]);
  ...
  const [осьТ,осьFs]=ужать(строка(дан&&дан.axis,ОБР_scat.axis),pw*.6,9,7,{s});
...
function elSpark(s,дан,зам){
  ...
  o+=txt(s,x+20,y+24,t(s,згТ),згFs,s.wl,s.ts,s.ls);
  const gl=s.glow?` filter="${s.glow}"`:'';
  const [ярл,яfs]=ужатьНабор(M.map(м=>t(s,м.l)),sx-lx-6,10.5,7,{s});
...
function elBubble(s,дан,зам){
  ...
  const B=ряды(дан&&дан.bubbles,ОБР_bub.bubbles,7).map((б,i)=>[
    ...
    вРамках(зам,`bubbles[${i}].x`,Number(б.x)||0,0,100),вРамках(зам,`bubbles[${i}].y`,Number(б.y)||0,0,100),
...
function elDumb(s,дан,зам){
  ...
  const [згТ,згFs]=ужать(заг,w-40,12,9,{сбор:зам,слот:'title',s,зазор:s.ls});
  o+=txt(s,x+20,y+24,t(s,згТ),згFs,s.wl,s.ts,s.ls);
  const [подписи,мfs]=ужатьНабор(rows.map(р=>t(s,String(р.label==null?'':р.label))),tx-lx-6,10,7,{s});
  rows.forEach((р,i)=>{
    const ty=top+i*шаг;
    ...
    const a=вРамках(зам,`rows[${i}].a`,Number(р.a)||0,0,100),b=вРамках(зам,`rows[${i}].b`,Number(р.b)||0,0,100);
...
function elArrows(s,дан,зам){
  ...
  const [имена,лfs]=ужатьНабор(L.map(п=>t(s,п[1])),aw-14,9.5,7,{s});
  L.forEach(([n2,l,c],i)=>{
...
function elFunnel(s,дан,зам){
  ...
  const L=ур.map((у,i)=>[String(у.label==null?'':у.label),String(у.value==null?'':у.value),
    вРамках(зам,`levels[${i}].k`,Number(у.k)||0,.08,1),пал[i%пал.length]]);
  ...
  const gl=s.glow?` filter="${s.glow}"`:'';
  const [метки,мfs]=ужатьНабор(L.map(п=>t(s,п[0])),labR-x-18,9.5,7,{сбор:зам,слоты:L.map((_,i)=>`levels[${i}].label`),s});
  L.forEach(([l,v,k,c],i)=>{
...
function elTimeline(s,дан,зам){
  ...
  const [подп,пfs]=ужатьНабор(P.map(п=>t(s,п[1])),шаг-6,9.5,7,{s});
...
function elCycle(s,дан,зам){
  ...
  const [ярл,яfs]=ужатьНабор(Q.map(п=>t(s,п[0])),46,9.5,7,{s});
  ...
  const [ц,цfs]=ужать(t(s,строка(дан&&дан.center,ОБР_cycle.center)),R*1.3,13,9,{s});
...
function elRoadmap(s,дан,зам){
  ...
  const R=ряды(дан&&дан.rows,ОБР_roadmap.rows,5)
    .map((р,i)=>[String(р.label==null?'':р.label),вРамках(зам,`rows[${i}].start`,Number(р.start)||0,0,1),
      вРамках(зам,`rows[${i}].len`,Number(р.len)||0,.04,1),пал[i%пал.length]]);
...
function elSwim(s,дан,зам){
  const x=PAD,y=PAD,w=W-PAD*2;
  const пал=[s.ac,s.ac3,s.ac2,s.pos];
  const L=ряды(дан&&дан.lanes,ОБР_swim.lanes,4)
    .map((п,i)=>[String(п.role==null?'':п.role),
      (Array.isArray(п.steps)?п.steps:[]).map((ш,j)=>[вРамках(зам,`lanes[${i}].steps[${j}].col`,Number(ш.col)||0,0,4),String(ш.label==null?'':ш.label)]),
      пал[i%пал.length]]);
  ...
  const [роли,рfs]=ужатьНабор(L.map(п=>t(s,п[0])),ox-x-12,9,6.5,{s});
  L.forEach(([role,steps,c],i)=>{
    ...
    steps.forEach(([col,lab])=>{
      ...
      const [labТ,labFs]=ужать(lab,sw2+4,8.5,6.5,{s});
...
function elTree2(s,дан,зам){
  ...
  /* Третий ярус композиции стоит строго под правой ветвью, поэтому потомки
     левой не рисуются никогда, а у правой рисуются только при ровно двух
     (договор: «0 или 2»). Раскладку не трогаем — она задумана такой; но
     переданное и выброшенное больше не пропадает молча */
  const дети=i=>{const б=переданы&&переданы[i];return (б&&Array.isArray(б.children))?б.children:[];};
  /* Подстановка образца поштучно — общее устройство библиотеки, но здесь она
     не видна ниоткуда: недостающее поле ветви подменяется образцовым, и в
     разметке оказывается текст, которого никто не передавал. Ярче всего это
     у children: branches[1] без них рисует образцовых «Своя»/«API» как свои.
     Пока данных нет вовсе, подстановка — ровно то, что нужно, и молчание
     верно; как только данные есть, о ней надо сказать. Про верхние поля
     говорит сверитьДанные, про поля внутри ветви не говорил никто. */
  const пусто=з=>з===undefined||з===null||з==='';
  if(переданы)обрB.forEach((об,i)=>{
    const б=переданы[i];
    const взято=(поле,нарисовано,почему)=>весть(зам,{'вид':'взято из образца',
      'слот':`branches[${i}].${поле}`,'нарисовано':нарисовано,'почему':почему});
    if(пусто(б&&б.label))взято('label',об.label,'поле не передано, подпись ветви взята из образца');
    if(пусто(б&&б.node))взято('node',об.node,'поле не передано, подпись узла взята из образца');
    // у левой ветви образцовых потомков нет, подменять нечем — и лгать нечем
    if(!(б&&Array.isArray(б.children))&&об.children.length>=2)
      взято('children',об.children.join(' / '),'поле не передано, третий ярус нарисован образцовыми потомками');
  });
  if(дети(0).length)
    весть(зам,{'вид':'не нарисовано','слот':'branches[0].children','сколько':дети(0).length,'нарисовано':0,
      'почему':'третий ярус стоит под правой ветвью: потомки левой не рисуются'});
  if(дети(1).length===1||дети(1).length>2)
    весть(зам,{'вид':'не нарисовано','слот':'branches[1].children','сколько':дети(1).length,
      'нарисовано':дети(1).length>2?2:0,
      'почему':'договор — 0 или 2 потомка; ярус рисуется только при двух'});
  ...
  const [rootТ,rootFs]=ужать(строка(дан&&дан.root,ОБР_dtree.root),rw-8,9,7,{s});
  o+=node(s,cx-rw/2,yR,rw,nh,s.ac,rootТ,rootFs);
  const [метки,мfs]=ужатьНабор([B[0].node,B[1].node],cw-8,9,7,{s});
  o+=node(s,cL-cw/2,yC,cw,nh,s.tm,метки[0],мfs);
  o+=node(s,cR-cw/2,yC,cw,nh,s.ac3,метки[1],мfs);
  if(внуки){
    const [лаб,лfs]=ужатьНабор(внуки,gw-8,8.5,6.5,{s});
    o+=node(s,gL-gw/2,yG,gw,nh,s.ac2,лаб[0],лfs);
    o+=node(s,gR-gw/2,yG,gw,nh,s.pos,лаб[1],лfs);
  }
  const [подп,пfs]=ужатьНабор([t(s,B[0].label),t(s,B[1].label)],44,8,6,{s});
...
function elLoop(s,дан,зам){
  ...
  const [ярлN,яfsN]=ужатьНабор(N.map(п=>t(s,п[0])),30,8.5,6,{s});
  N.forEach(([l,i])=>{
...
function elJourney(s,дан,зам){
  ...
  const ST2=ряды(дан&&дан.stages,ОБР_journey.stages,6)
    .map((ст,i)=>[String(ст.label==null?'':ст.label),вРамках(зам,`stages[${i}].mood`,Number(ст.mood)||0,1,5)]);
  ...
  const [подп,пfs]=ужатьНабор(ST2.map(ст=>t(s,ст[0])),шаг-6,8.5,6.5,{s});
...
function elTCards(s,дан,зам){
  ...
  o+=txt(s,x+20,y+24,t(s,згТ),згFs,s.wl,s.ts,s.ls);
  const gl=s.glow?` filter="${s.glow}"`:'';
  const [тексты,тfs]=ужатьНабор(C.map(п=>t(s,п[1])),cw-22,9,7,{s});
...
function elPyramid(s,дан,зам){
  ...
  L.forEach(([l,c],i)=>{
    ...
    const [lТ,lFs]=ужать(t(s,l),maxW-16,9.5,7,{s});
...
function elSwot(s,дан,зам){
  ...
  const [ярлQ,яfsQ]=ужатьНабор(Q.map(п=>t(s,п[1])),qw-24,9,6.5,{s});
  const gl=s.glow?` filter="${s.glow}"`:'';
...
function elM22(s,дан,зам){
  ...
  const [ярлM,яfsM]=ужатьНабор(Q.map(п=>t(s,п[0])),pw/2-24,9,6.5,{s});
  ...
  const [axYТ,axYFs]=ужать(строка(дан&&дан.axisY,ОБР_m22.axisY),44,8.5,6.5,{s});
  o+=txt(s,x+22,oy+ph/2,t(s,axYТ),axYFs,s.wl,s.tm,null).replace('<text ','<text text-anchor="middle" ');
  const [axXТ,axXFs]=ужать(строка(дан&&дан.axisX,ОБР_m22.axisX),pw,8.5,6.5,{s});
...
function elVenn(s,дан,зам){
  ...
  const [стрЦ,цfs]=ужатьНабор(центр.map(с=>t(s,с)),2*r-d,8.5,7,{s});
...
function elFMatrix(s,дан,зам){
  ...
  const [згТ,згFs]=ужать(заг,w-40,12,9,{сбор:зам,слот:'title',s,зазор:s.ls});
  o+=txt(s,x+20,y+24,t(s,згТ),згFs,s.wl,s.ts,s.ls);
  const [шапка,шFs]=ужатьНабор(P.map(п=>t(s,п)),colW,8.5,6.5,{s});
  const [строки,сfs]=ужатьНабор(F.map(ф=>t(s,ф[0])),cx0-lx-colW/2-8,9,6.5,{s});
...
function elNet(s,дан,зам){
  ...
  const [ярлL,яfsL]=ужатьНабор(обрL.map((об,i)=>t(s,String(сл[i]&&сл[i].label!=null?сл[i].label:об))),84,8.5,6.5,{s});
  ярлL.forEach((l,i)=>
...
function elTrans(s,дан,зам){
  ...
  const [ярлR,яfsR]=ужатьНабор(R.map(п=>п[0]),bw-16,8.5,6.5,{s});
  R.forEach(([l,c],i)=>{
...
function elRag(s,дан,зам){
  ...
  const [ярлR,яfsR]=ужатьНабор(R.map(п=>п[0]),aw-6,9,6.5,{s});
  R.forEach(([l,c],i)=>{
    const px=bx+i*(aw+11);
  ...
  const [храп,хfs]=ужать(строка(дан&&дан.store,ОБР_rag.store),vw-8,8.5,6.5,{s});
  ...
  const [ноТ,ноFs]=ужать(строка(дан&&дан.note,ОБР_rag.note),w-40,8,6.5,{s});
...
function elLLM(s,дан,зам){
  ...
  const M=ряды(дан&&дан.models,ОБР_llm.models,5)
    .map((м,i)=>[String(м.name==null?'':м.name),String(м.params==null?'':м.params),
      вРамках(зам,`models[${i}].score`,Number(м.score)||0,0,100),пал[i%пал.length]]);
  const lx=x+20, px=x+96, pw=w-160, rh=26, top=y+42;
  const [имена,иfs]=ужатьНабор(M.map(м=>t(s,м[0])),px-lx-8,10,7,{s});
...
function elVec(s,дан,зам){
  ...
  CL.forEach((cl,i)=>{
    ...
    if(i<2){
      ...
      const [клТ,клFs]=ужать(t(s,l),budget,9,7,{s});
  ...
  const [примТ,примFs]=ужать(прим,w-40,8,6.5,{s});
...
function elMicro(s,дан,зам){
  ...
  const [ярлS,яfsS]=ужатьНабор(S.map(п=>п[0]),sw2-6,8.5,6.5,{s});
  const dy=sy+36;
  S.forEach((_,i)=>{const sx=x+26+i*(sw2+9);
    o+=conn(s,gx+gw/2,gy+bh,sx+sw2/2,sy);});
  const [шлз,шfs]=ужать(строка(дан&&дан.gateway,ОБР_micro.gateway),gw-10,9,7,{s});
...
function elCicd(s,дан,зам){
  ...
  const [ярлP,яfsP]=ужатьНабор(P.map(п=>t(s,п[0])),bw-4,7.5,6,{s});
  ...
  const прог=вРамках(зам,'progress',дан&&дан.progress!=null?Number(дан.progress):ОБР_cicd.progress,0,1);
  o+=track(s,x+26,py,pw,7);
  o+=shape(s,x+26,py,pw*прог,7,s.pos,1);
  const прим=строка(дан&&дан.note,ОБР_cicd.note);
  const [примТ,примFs]=ужать(прим,pw,8.5,7,{s});
...
function elEgov(s,дан,зам){
  ...
  const [акт,аfs]=ужать(строка(дан&&дан.actor,ОБР_egov.actor),cw-8,8.5,6.5,{s});
  o+=node(s,cx-cw/2,top,cw,bh,s.ac3,акт,аfs);
  o+=`<line x1="${cx}" y1="${top+bh}" x2="${cx}" y2="${top+bh+12}" stroke="${s.ln}" stroke-width="1.4"/>`;
  const hy=top+bh+12;
  const [шлюз,шfs]=ужать(строка(дан&&дан.hub,ОБР_egov.hub),w-70,9,7,{s});
  ...
  const [ярлM2,яfsM2]=ужатьНабор(M.map(п=>п[0]),mw-6,8.5,6.5,{s});
  ...
  const [примТ,примFs]=ужать(прим,w-40,7.5,6.5,{s});
  o+=txt(s,x+20,y+h-18,t(s,примТ),примFs,s.wl,s.tm,null);
  return wrapSvg(s,o);
}
...
function elBus(s,дан,зам){
  ...
  const [ярлPR,яfsPR]=ужатьНабор(PR.map(п=>п[0]),bw-6,8,6,{s});
  const [ярлCO,яfsCO]=ужатьНабор(CO.map(п=>п[0]),bw-6,8,6,{s});
  ...
  const [шина,шfs]=ужать(строка(дан&&дан.bus,ОБР_bus.bus),w-70,9,7,{s});
...
function elLayers(s,дан,зам){
  ...
  const [примТ,примFs]=ужать(прим,bw,7.5,6.5,{s});
  ...
  const [имена,иfs]=ужатьНабор(L.map(п=>t(s,п[0])),bw*.45,9.5,7,{s});
  const [стеки,сfs]=ужатьНабор(L.map(п=>t(s,п[1])),bw*.52,8,6,{s});
  L.forEach(([l,sub,c],i)=>{
    const by=top+i*(bh+g);
...
function elBigNum(s,дан,зам){
  ...
  const [eyebrowТ,eyebrowFs]=ужать(eyebrow,maxW,9.5,null,{s,зазор:2.2});
  ...
  const [captionТ,captionFs]=ужать(caption,maxW,11,null,{s});
  o+=txt(s,cx,y+120,t(s,captionТ),captionFs,s.wl,s.ts,null).replace('<text ','<text text-anchor="middle" ');
  const [deltaТ,deltaFs]=ужать(delta,maxW,9.5,null,{s});
...
function elFacts(s,дан,зам){
  ...
  const [подписи,пfs]=ужатьНабор(F.map(ф=>t(s,ф[1])),cw-16,9,6.5,{s});
  F.forEach(([v,l,c],i)=>{
...
function elStatCard(s,дан,зам){
  ...
  const [labelТ,labelFs]=ужать(label,w-48,9.5,null,{s,зазор:1.8});
  ...
  const [deltaТ,deltaFs]=ужать(delta,w-48,10,null,{s});
  ...
  const [noteТ,noteFs]=ужать(note,w-48,9,null,{s});
...
function elRing(s,дан,зам){
  ...
  const V=вРамках(зам,'value',дан&&дан.value!=null&&!Number.isNaN(Number(дан.value))?Number(дан.value):ОБР_ring.value,0,100);
  ...
  const [имена,иfs]=ужатьНабор(лег.map(л=>t(s,л[0])),avail,9.5,7,{s});
  const [субт,сfs]=ужатьНабор(лег.map(л=>t(s,л[1])),avail,8.5,6.5,{s});
  лег.forEach(([,,c],i)=>{
...
function elIcons(s,дан,зам){
  ...
  const [подписи,пfs]=ужатьНабор(I.map(э=>t(s,э[1])),cw-10,9,6.5,{s});
  I.forEach(([v,l,c],i)=>{
  ...
  const [примТ,примFs]=ужать(прим,w-40,8.5,7,{s});
  o+=txt(s,x+20,y+h-18,t(s,примТ),примFs,s.wl,s.tm,null);
  return wrapSvg(s,o);
}
...
function elQuote(s,дан,зам){
  ...
  const [строкиЦ,цfs]=ужатьНабор(цит.map(с=>t(s,с)),x+w-22-(lx+24),11.5,8.5,{s});
  строкиЦ.forEach((с,i)=>o+=txt(s,lx+24,y+34+i*16,с,цfs,s.wl,s.tp,null));
  const авт=строка(дан&&дан.author,ОБР_quote.author);
  const [автТ,автFs]=ужать(авт,x+w-22-(lx+24),9,7,{s});
  ...
  const [подписиЦ,пfsЦ]=ужатьНабор(S.map(э=>t(s,э[1])),cw-8,8.5,6.5,{s});
  S.forEach(([v,l,c],i)=>{
...
function elBefore(s,дан,зам){
  ...
  const [,сfs]=ужатьНабор([...было,...стало].map(в=>t(s,в)),cw-12,11,7.5,{s});
  cols.forEach(([hd,c,rows,hi])=>{
    ...
    rows.forEach((v,ri)=>o+=txt(s,bx+cw/2,r0+ri*rs,ужать(t(s,v),cw-12,сfs,сfs,{s})[0],сfs,hi?700:s.wl,tc,null).replace('<text ','<text text-anchor="middle" '));
  });
...
function elDivider(s,дан,зам){
  ...
  const [eyebrowТ,eyebrowFs]=ужать(eyebrow,maxW,9,null,{s,зазор:3});
  ...
  const [згТ,згFs]=ужать(заг,maxW,18,null,{сбор:зам,слот:'title',s});
  ...
  const [subТ,subFs]=ужать(sub,maxW,9.5,null,{s});
...
function elChapter(s,дан,зам){
  ...
  const [eyebrowТ,eyebrowFs]=ужать(eyebrow,maxW,9,null,{s,зазор:2.6});
  ...
  const [стрГ,гfs]=ужатьНабор(строки.map(с=>t(s,с)),maxW,10.5,7,{s});
  ...
  const [metaТ,metaFs]=ужать(meta,maxW,9,null,{s});
...
function elThanks(s,дан,зам){
  ...
  const [згТ,згFs]=ужать(заг,maxW,34,null,{сбор:зам,слот:'title',s});
  ...
  const [имяТ,имяFs]=ужать(имя,maxW,12,null,{s});
  o+=txt(s,cx,y+100,t(s,имяТ),имяFs,600,s.ts,null).replace('<text ','<text text-anchor="middle" ');
  const [оргТ,оргFs]=ужать(орг,maxW,9.5,null,{s});
...
function elAgenda(s,дан,зам){
  ...
  const [згТ,згFs]=ужать(заг,w-44,13,10,{сбор:зам,слот:'title',s});
  ...
  const [пп,пfs]=ужатьНабор(A.map(п=>t(s,п[1])),x+w-22-(x+46),10.5,8,{s});
  A.forEach(([n2,l,c],i)=>{
    const ly=y+56+i*21;
...
function elSpeaker(s,дан,зам){
  ...
  const [имяТ,имяFs]=ужать(имя,maxW,14,null,{s});
  o+=txt(s,lx,y+50,t(s,имяТ),имяFs,s.wv,s.mode==='neon'||s.mode==='retro'?s.ac:s.tp,-.2);
  const [рольТ,рольFs]=ужать(роль,maxW,10,null,{s});
  ...
  const [стр1Т,стр1Fs]=ужать(стр1,maxW,9,null,{s});
  o+=txt(s,lx,y+118,t(s,стр1Т),стр1Fs,s.wl,s.ts,null);
  const [стр2Т,стр2Fs]=ужать(стр2,maxW,9,null,{s});
...
function elFaq(s,дан,зам){
  ...
  /* низ считается по последнему видимому: у свёрнутого вопроса плашки нет,
     остаётся только строка с маркером «+/−». Маркер рисуется кеглем 13
     (фиксированным, не тем же, что у подписи вопроса) и потому имеет
     больший выносной элемент — именно он, а не текст вопроса, определяет
     фактический визуальный низ строки. Прежняя оценка кеглем 9.5
     (кегль подписи, не маркера) занижала выносной на 0.77px — нижний
     отступ у последнего свёрнутого пункта выходил 15.2px при норме 16 */
  let низQ=top, ходQ=top;
  Q.forEach(([,open])=>{const в=open?42:26;
    низQ=Math.max(низQ,open?ходQ+в:ходQ+16+13*0.22); ходQ+=в+6;});
  const hh=рост(низQ);   // раскрытых больше одного — холст растёт
  const h=hh-PAD*2;
  let o=frameH(s,hh)+surface(s,x,y,w,h);
  const заг=строка(дан&&дан.title,ОБР_faq.title);
  const [згТ,згFs]=ужать(заг,w-44,12,9,{сбор:зам,слот:'title',s,зазор:s.ls});
  o+=txt(s,x+22,y+28,t(s,згТ),згFs,s.wl,s.ts,s.ls);
  const [вопр,вfs]=ужатьНабор(Q.map(п=>t(s,п[0])),bw-40,9.5,7,{s});
  let cy=top;
  Q.forEach(([q,open,ans],i)=>{
    ...
    if(open){const [ansТ,ansFs]=ужать(ans,bw-28,8.5,7,{s});o+=txt(s,bx+14,cy+32,t(s,ansТ),ansFs,s.wl,s.tm,null);}
    cy+=bh+6;
