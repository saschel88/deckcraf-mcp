...
/* ══════ МЕРА ТЕКСТА ══════
   ...
   рисуется запасной. Замер задачи 32 назвал его отрисовкой в браузере,
   а не по догадке: поглифная таблица 139 знаков у нашего стека совпала
   с Segoe UI знак в знак, а у моностека — с Consolas. Раз шрифт зрителя
   неизвестен, мера обязана ЗАВЫШАТЬ: тогда текст ужмётся раньше времени,
   но не вылезет.

   ── ШКАЛ ДВЕ, ПОТОМУ ЧТО СЕМЕЙСТВ ДВА ──

   Пропорциональный стек (двенадцать стилей из четырнадцати) — четыре класса
   и метрики Arial, он шире Segoe UI из двух:
   ...
   Моноширинный стек (neon, retro) — 0.55 НА ЛЮБОЙ ЗНАК. У моноширинного
   шрифта все знаки одной ширины по определению, и четыре класса на нём
   врали грубо: замер 34 намерил перебор +20.0% в среднем и среднюю
   абсолютную ошибку 21.7%. С единой шириной — 1.1% и −0.4%: прописные
   больше не ужимаются раньше времени. Число проверено браузером:
   у Consolas 137 знаков из 139 ровно 0.5498.

   ПОЧЕМУ ИМЕННО ЭТО РАЗЛИЧЕНИЕ, А НЕ ПОГЛИФНАЯ ТАБЛИЦА ПО НАСТОЯЩИМ
   ШРИФТАМ. Таблица точнее вдвое с лишним (0.5% против 10.9%) — но ровно
   до первого зрителя с другим шрифтом: против Verdana та же таблица даёт
   8.8% и занижает в 94% случаев, то есть проигрывает по занижению даже
   нынешней мере (замер 34, десять установленных sans-шрифтов). Обещание
   «точно» она держит только перед Windows-зрителем с Chrome. Моноширинность
   же ОТ ШРИФТА НЕ ЗАВИСИТ: моноширинный шрифт таков везде, и 0.55 верно
   и для Consolas, и для Courier New, и для Cascadia Mono, и для Lucida
   Console. Выгода не привязана к машине, на которой мерили.

   ЧЕГО МЕРА ПО-ПРЕЖНЕМУ НЕ ЗНАЕТ. Сказано здесь, чтобы читающий не принял
   молчание за отсутствие беды:
     • ВЕС. Лиц не два, а четыре: Segoe UI при весе 700 против 500 шире
       по медиане в 1.039 раза, при 900 — в 1.079, на краю в 1.326.
       Веса мера не получает вовсе — в txt() он идёт отдельным доводом
       и сюда не доходит. Остаток назван, не закрыт.
     • ЗАНИЖЕНИЕ НА SANS. 53% узлов на Segoe UI мера занижает, до −36%
       на знак: в классе «прочее» лежат и «.» (0.241), и «m» (0.886).
       Поднять классы можно, и цена померена — подъём даже на 5% сужает
       1710 договорных пределов из 2067, а «не занижать нигде» стоит
       автору 1244 обрезанных подписей при среднем переборе +62%.
     • 12% ВЫЗОВОВ НЕ ЗНАЮТ СЕМЕЙСТВА. Место вызова передаёт признаки
       не всегда (1794 вызова из 14998 на образцовом корпусе): там `s` нет,
       семейство неизвестно, и мера остаётся четырёхклассовой, то есть
       завышает. Это безопасная сторона ошибки, и она нарочная.

   Два признака мера получает от места вызова, потому что из самой строки
   их не видно:
     s      — стиль. Из него берётся и регистр (семь стилей из тринадцати
              поднимают его, и делает это t() уже ПОСЛЕ замера,
              engine.js:283: мера смотрела на строчные буквы, а холст
              получал прописные), и СЕМЕЙСТВО — `s.ff`, то самое, что
              уходит в разметку атрибутом font-family.
...
const ШИР_МОНО = 0.55;                                                       // spec/tokens.json
/* Семейство читается по стеку, а не по списку имён стилей: стили заводят
   и переименовывают, а «в стеке названо моноширинное семейство» — свойство
   самого объявления. Ловит и `ui-monospace`, и `monospace`, и `'SF Mono'`. */
...
const мера = (str, fs, оп) => {
  ...
  const моно = !!(оп && оп.s && моноширинный(оп.s.ff));
  return [...т].reduce((ш, c) => ш + (моно ? ШИР_МОНО : классЗнака(c)), 0) * fs + зз * т.length;
};
...
function словами(з) {
  switch (з['вид']) {
    ...
    case 'дополнено_образцом':
      return `${з['слот']}: нарисовано «${з['нарисовано']}» — это значение образца, а не переданное ` +
        `(${з['почему']})`;
    case 'не нарисовано':
...
/* ══════ ИМЯ СЛОТА ТОЛЬКО ДЛЯ ПЕРЕДАННОГО ══════
   Наряд 29. Запись «не влезло» — указание модели-сборщику сократить поле.
   Про поле, которого она не присылала, такое указание не просто бесполезно:
   оно врёт. А нарисовано может быть именно образцовое — ряды() подставляют
   его целым массивом, когда поля нет вовсе, а девять элементов подставляют
   поштучно (elNet: `сл[i] && сл[i].label != null ? … : образец`), собирая
   набор наполовину из переданного, наполовину из ОБР_*.

   И это не теория: четырнадцать мест подгонки обрезают СОБСТВЕННЫЙ образец —
   у swim во всех тринадцати стилях, у rag, speaker, egov и venn в семи с
   капсом. Без стража ниже каждая отрисовка без данных давала бы полный
   список замечаний о тексте, которого никто не передавал.

   Отсюда два спутника ряды() и строка(), по одному на каждую: слот
   называется тогда и только тогда, когда нарисовано ПЕРЕДАННОЕ. Иначе имя
   слота — null, и переполнение() молчит своим же существующим стражем,
   второй ветки в нём не заведено.

   Смотрят они на СЫРОЕ поле «дан», а не на итог ряды(): у итога уже не
   спросишь, чей это ряд. Условие у «вРяду» — ровно то же `!= null`, которым
   места вызова выбирают между переданным и образцовым, а не своё: разойдись
   они, страж молчал бы там, где нарисовано переданное. */
const своё = з => з !== undefined && з !== null && з !== '';
const вРяду = (д, i, поле) => Array.isArray(д) && д[i] != null &&
  (поле === undefined ? true : д[i][поле] != null);
...
/* ══════ ПРЕДЕЛ ДЛИНЫ ПОЛЯ — СЧИТАЕТСЯ ПОД ВЫБРАННЫЙ СТИЛЬ ══════
   Сколько знаков поле держит без многоточия. Число это стилевое, а не общее:
   семь стилей из тринадцати поднимают регистр (t(), engine.js:283), у тех же
   семи межбуквенный зазор 1.4–2.4 на знак, и один и тот же слот берёт от 26
   знаков в line до 44 в flat. Единого честного числа на тринадцать стилей
   не существует — а выбирать его и не нужно: стиль в сессии зафиксирован
   раньше всего, без start не работают ни find, ни describe, ни draw.

   ВТОРОЙ МЕРЫ ЗДЕСЬ НЕТ, И ЭТО ГЛАВНОЕ. Предел не выводится повторением
   формулы ширины — он снимается с самой библиотеки: элемент рисуется
   настоящим стилем на пробной строке, и в разметке смотрят, доехала ли она
   целиком. Значит внутри работают те же ужать()/ужатьБезТочек(), та же мера,
   тот же пол кегля, та же ширина слота и тот же t() с его капсом. Двойников
   меры в проекте было три, и они разъезжались; четвёртого не будет.

   Проба — настоящая русская фраза смешанного регистра, а не выдуманный ряд
   знаков: ширина знака теперь разная (прописная 0.73, строчная 0.55, цифра
   0.556, пробел 0.28), и предел, снятый на ровном материале, не описывал бы
   ни один живой заголовок. Капс при этом учтён без единой оговорки: в стилях
   с caps подпись поднимает сам элемент, замер видит уже поднятую.

   Остаточная неточность названа честно: если автор пришлёт СПЛОШЬ ПРОПИСНЫЕ
   в стиле без капса, они шире пробы и могут не влезть. Ловить это заранее
   не нужно — с наряда 12 у нас есть машиночитаемая запись «не влезло»
   с требуемой и доступной шириной, и предел обязан быть хорошим первым
   приближением, а не гарантией на любой мыслимый текст.

   Числа памятны: предел зависит только от элемента, поля и стиля,
   а не от переданных данных, и снимается один раз на пару. */
const ПРОБА_ПРЕДЕЛА = ('Показатель эффективности работы подразделения за отчётный период ' +
  'текущего года и следующего квартала по данным ведомственной отчётности ').repeat(3);
const ВЕРХ_ПРЕДЕЛА = 140;                        // выше не меряем: самый ёмкий слот держит 58

/* Разметку читаем без разбора XML: у нас нет ни tspan, ни вложенности —
   txt() кладёт подпись одним текстовым узлом, и этого хватает. */
const узлыТекста = svg => [...String(svg).matchAll(/<text\b[^>]*>([^<]*)<\/text>/g)].map(м => м[1]);

/* Доехала ли строка целиком. Узлы идут по порядку разметки; ищем первый,
   что является её началом, и доедаем им остаток — так ловится и однострочная
   подпись, и разбитая переносом на слова. Обрезка любого рода рвёт цепочку:
   и многоточие ужать(), и молчаливое усечение ужатьБезТочек(). */
function цела(узлы, ожид) {
  for (let i = 0; i < узлы.length; i++) {
    if (!узлы[i] || !ожид.startsWith(узлы[i])) continue;
    let ост = ожид.slice(узлы[i].length), j = i + 1;
    while (ост.length && j < узлы.length) {
      if (ост[0] === ' ') ост = ост.slice(1);
      if (!узлы[j] || !ост.startsWith(узлы[j])) break;
      ост = ост.slice(узлы[j].length); j++;
    }
    if (!ост.length) return true;
  }
  return false;
}
/* Ждём подпись в обоих регистрах: часть полей идёт через t() и в семи стилях
   поднимается, часть рисуется как передано (kpi.value). Спрашивать только
   поднятый регистр значило бы объявить «не влезло» там, где текст цел. */
const доехала = (svg, проба) => { const у = узлыТекста(svg); return цела(у, проба) || цела(у, проба.toUpperCase()); };

const памятьПределов = new Map();
const ячейка = (м, к) => { let в = м.get(к); if (!в) м.set(к, в = new Map()); return в; };

/* МАССИВ СТРОК: тот же замер, но проба — НАБОР из «строк» строк.
   Второй меры здесь по-прежнему нет: расширен вход существующего —
   тот же двоичный поиск, та же пробная фраза, тот же признак «доехала».
   Причина отдельного входа: у массива предел зависит ещё и от числа строк —
   ужатьНабор() выдаёт всему набору общий кегль по самой длинной строке,
   а ширина слота у половины полей делится на их число. Парой поле/стиль
   такой предел не описывается.

   СТРОКИ РАЗЛИЧИМЫ, И ЭТО ГЛАВНОЕ. На одинаковых замер врёт: если слоты
   разной ширины (vec.clusters), одного уцелевшего узла хватает, чтобы
   объявить целыми все три, — оттуда и брались 21..28 вместо 12..16.
   Различаются строки ПЕРВЫМ ЗНАКОМ и только им: метки — прописные
   кириллические буквы, того же класса ширины (0.73), что и заменяемая ими
   «П» пробной фразы. Значит строки равношироки во всех тринадцати стилях
   и при капсе тоже — предел мерится на ровном наборе, а не на случайном
   перекосе ширин, но каждая строка опознаётся в разметке без путаницы
   с соседками. Целыми обязаны доехать ВСЕ строки. */
const МЕТКИ_СТРОК = 'АБВГДЕЖЗИК';
const наборПробы = (n, строк) => Array.from({ length: строк }, (_, i) =>
  МЕТКИ_СТРОК[i % МЕТКИ_СТРОК.length] + ПРОБА_ПРЕДЕЛА.slice(1, n));

/* ПОДПОЛЕ ВНУТРИ МАССИВА ОБЪЕКТОВ: тот же замер, тот же двоичный поиск,
   та же пробная фраза, тот же признак «доехала». Расширен ВХОД, а не
   заведена вторая мера — как это сделал наряд 20 для набора строк.
   «поле» теперь может быть путём через точку — «rows.label»,
   «lanes.steps.label», — а «строк» по-прежнему значит число элементов
   ВЕРХНЕГО массива пути (у него же в договоре стоит «количество»).

   ПОЧЕМУ БЕЗ ОБРАЗЦА ТУТ НЕ ОБОЙТИСЬ. Скалярное поле подавалось в отрисовку
   в одиночку: всё прочее элемент берёт из ОБР_* сам через ряды()/строка().
   У подполя так не выйдет — подставить надо весь массив целиком, а вместе
   с ним и соседние подполя каждого ряда: bars.rows.v задаёт длину полосы,
   swim.lanes.steps.col — колонку, funnel.levels.k — ширину полосы. Берём их
   из образца элемента: ряд образца клонируется по кругу, и заменяется в нём
   ровно одно подполе. Отсюда то же правило чтения, что и у скалярного
   замера, слово в слово: ПРЕДЕЛ МЕРЯЕТСЯ ПО ОБРАЗЦУ ЧУЖИХ ПОЛЕЙ.

   ЗАМЕНЯЮТСЯ ВСЕ СЛОТЫ СРАЗУ, И ДОЕХАТЬ ОБЯЗАНЫ ВСЕ. Подписи набора идут
   через ужатьНабор() — общий кегль по самой длинной строке, — а слоты
   у части элементов разной ширины (tree.blocks, bub.bubbles, funnel.levels).
   Заменить один слот значило бы мерить самый широкий и молчать про остальные;
   число по самому тесному слоту — то же решение, что принял наряд 20
   для vec.clusters. Строки различимы первым знаком, как в наборПробы:
   на одинаковых замер удовлетворяется одним уцелевшим узлом и врёт вдвое.

   СЛОТ, КОТОРОГО В РАЗМЕТКЕ НЕТ ВОВСЕ, В ЗАМЕР НЕ ВХОДИТ. Это не поблажка,
   а то же правило, по которому скалярный предел бывает null («в этом стиле
   поле подписью не рисуется»), — только применённое к каждому слоту
   в отдельности. Живых случаев два, и оба договорные:
     faq.items.a       — ответ рисуется только у раскрытого пункта (open),
                         у свёрнутых его в разметке нет;
     bub.bubbles.label — подпись рисуется только при r > 14, мелкому
                         пузырю её не втискивают (так и написано в элементе).
   Требуй замер всех слотов подряд — оба поля дали бы null, то есть договор
   объявил бы длину тому, чего в разметке нет. Живые слоты определяются
   один раз, на пробе в один знак: короче не бывает, и если слота нет
   и там — его нет вообще. Все живые обязаны доехать целыми. */
let ОБРАЗЦЫ = null;
const образецЭлемента = fn => {
  if (!ОБРАЗЦЫ) { ОБРАЗЦЫ = new Map(); CATS.forEach(([, сп]) => сп.forEach(([, , f, о]) => { if (!ОБРАЗЦЫ.has(f)) ОБРАЗЦЫ.set(f, о); })); }
  return ОБРАЗЦЫ.get(fn) || null;
};
const клонД = о => (о === null || typeof о !== 'object') ? о
  : Array.isArray(о) ? о.map(клонД)
  : Object.fromEntries(Object.entries(о).map(([к, з]) => [к, клонД(з)]));

/* Данные под замер подполя: [объект для отрисовки, список слотов].
   Слот — пара [куда, ключ-или-индекс]; проба ляжет в каждый. Лист-массив
   (dtree.branches.children — массив строк внутри объекта) даёт по слоту
   на элемент, лист-строка — один. null, если образец такого пути не имеет:
   мерить нечего, и притворяться числом нельзя. */
function слотыПодполя(обр, путь, строк) {
  if (!обр) return null;
  const исходный = обр[путь[0]];
  if (!Array.isArray(исходный) || !исходный.length) return null;
  const N = Number.isInteger(строк) && строк > 0 ? строк : исходный.length;
  const собранные = [], слоты = [];
  const спуск = (узел, ост) => {
    if (!узел || typeof узел !== 'object') return;
    if (ост.length === 1) {
      const тек = узел[ост[0]];
      if (Array.isArray(тек)) тек.forEach((_, i) => слоты.push([тек, i]));
      else слоты.push([узел, ост[0]]);
      return;
    }
    const вн = узел[ост[0]];
    if (Array.isArray(вн)) вн.forEach(э => спуск(э, ост.slice(1)));
  };
  for (let i = 0; i < N; i++) {
    const ряд = клонД(исходный[i % исходный.length]);
    собранные.push(ряд);
    спуск(ряд, путь.slice(1));
  }
  return слоты.length ? [{ [путь[0]]: собранные }, слоты] : null;
}

/* Готовый материал замера подполя: что подать в отрисовку и какие строки
   искать потом в разметке. Отдаётся наружу нарочно — проба в data-check.js
   обязана мерить ТО ЖЕ САМОЕ, а расходиться с библиотекой ей положено
   только признаком «влезло», иначе она станет тавтологией. */
const поставитьСоседей = (данные, соседи) => {
  for (const [путь, зн] of Object.entries(соседи || {})) {
    const ост = String(путь).split('.');
    const верх = данные[ост[0]];
    if (!Array.isArray(верх)) continue;
    const спуск = (узел, о) => {
      if (!узел || typeof узел !== 'object') return;
      if (о.length === 1) { узел[о[0]] = зн; return; }
      const вн = узел[о[0]];
      if (Array.isArray(вн)) вн.forEach(э => спуск(э, о.slice(1)));
    };
    верх.forEach(р => спуск(р, ост.slice(1)));
  }
};

const пробаПодполя = (отрисовка, поле, длина, строк, соседи) => {
  const г = слотыПодполя(образецЭлемента(отрисовка), String(поле).split('.'), строк);
  if (!г) return null;
  const [данные, слоты] = г;
  поставитьСоседей(данные, соседи);
  const проба = наборПробы(длина, слоты.length);
  слоты.forEach(([куда, к], j) => { куда[к] = проба[j]; });
  return { данные, проба };
};

/* ПРАВИЛО, КОТОРОЕ НАДО ЗНАТЬ, ПРЕЖДЕ ЧЕМ ВЕРИТЬ ЭТОМУ ЧИСЛУ:
   ПРЕДЕЛ МЕРЯЕТСЯ ПО ОБРАЗЦУ ЧУЖИХ ПОЛЕЙ. Замер подаёт в отрисовку ровно
   одно поле — измеряемое, — а всё остальное берётся из ОБР_* через ряды()
   и строка(). Значит вся геометрия, зависящая от СОСЕДНИХ полей, взята
   из образца, а не из тех данных, о которых предупреждают.

   Это не мелочь, и вот три живых случая:
     heat.cols     — ширина ячейки считается от числа столбцов МАТРИЦЫ
                     (нc = max длин строк matrix), а не от длины самого cols:
                     замер видит образцовые 4×5, шесть столбцов уронили бы
                     предел с шести знаков до пяти;
     area.labels   — число подписанных точек к = min(длина labels, число
                     ТОЧЕК), и точки при замере образцовые;
     любой элемент — высота и шаг ряда считаются от числа элементов соседних
                     массивов, а они при замере образцовые.

   Отсюда правило чтения: предел — хорошее первое приближение под выбранный
   стиль, а не гарантия под любой набор соседних данных. Настоящую беду
   ловит не он, а машиночитаемая запись «не влезло» из переполнение():
   она снимается с той самой отрисовки, где все поля настоящие. Меру
   с полными данными сюда не заводить: она зависела бы от всего объекта
   сразу, памятной по [отрисовка, поле, стиль] быть перестала бы, а describe,
   который зовут ДО данных, не смог бы ответить вовсе.

   [отрисовка, поле, стиль, число строк] → предел в знаках, либо null, если
   поле в этом стиле подписью не рисуется (у retro дельта kpi — шкала из
   блоков, у aurora плашки нет вовсе). Четвёртый довод необязателен: без него
   мерится скалярная строка, как и прежде, знак в знак. «поле» с точкой —
   путь к подполю массива объектов, тоже знак в знак к прежнему поведению
   у имени без точки.
   Двоичный поиск: порядок монотонен, и это не допущение —
   проба в tools/data-check.js проверяет его сплошным перебором. */
function пределЗнаков(отрисовка, поле, s, строк, соседи) {
  if (typeof отрисовка !== 'function' || отрисовка.length < 2 || !s) return null;
  const набором = Number.isInteger(строк) && строк > 0;
  const путь = String(поле).split('.');
  const подполем = путь.length > 1;
  const ключ = набором ? `${поле}#${строк}` : поле;
  const пам = ячейка(ячейка(памятьПределов, отрисовка), s);
  /* Память — только для замера ПО ОБРАЗЦУ. С соседями её нет нарочно:
     значения приходят из договора и из данных автора, и ключ по ним
     рос бы неограниченно у долго живущего сервера. Замер и без памяти
     стоит один двоичный поиск. */
  if (!соседи && пам.has(ключ)) return пам.get(ключ);
  let маска = null;
  if (подполем) {
    const м1 = пробаПодполя(отрисовка, поле, 1, строк, соседи);
    if (!м1) { if (!соседи) пам.set(ключ, null); return null; }
    let svg1 = ''; try { svg1 = отрисовка(s, м1.данные, []); } catch { /* нет разметки — нет и живых слотов */ }
    маска = м1.проба.map(р => доехала(svg1, р));
    if (!маска.some(Boolean)) { if (!соседи) пам.set(ключ, null); return null; }
  }
  const влезает = n => {
    if (подполем) {
      /* Пересобираем на каждый шаг: слоты — живые ссылки внутрь данных,
         и подстановка их портит. Клон образца дешевле путаницы. */
      const м = пробаПодполя(отрисовка, поле, n, строк, соседи);
      if (!м) return false;
      let svg; try { svg = отрисовка(s, м.данные, []); } catch { return false; }
      return м.проба.every((р, i) => !маска[i] || доехала(svg, р));
    }
    const проба = набором ? наборПробы(n, строк) : ПРОБА_ПРЕДЕЛА.slice(0, n);
    let svg; try { svg = отрисовка(s, { [поле]: проба }, []); } catch { return false; }
    return набором ? проба.every(р => доехала(svg, р)) : доехала(svg, проба);
  };
  let итог = null;
  if (влезает(1)) {
    if (влезает(ВЕРХ_ПРЕДЕЛА)) итог = ВЕРХ_ПРЕДЕЛА;
    else {
      let низ = 1, верх = ВЕРХ_ПРЕДЕЛА;          // низ влезает, верх — нет
      while (верх - низ > 1) { const с = (низ + верх) >> 1; if (влезает(с)) низ = с; else верх = с; }
      итог = низ;
    }
  }
  if (!соседи) пам.set(ключ, итог);
  return итог;
}

/* ── ГРАНИЦЫ СОСЕДНЕГО ЧИСЛА И ХУДШЕЕ ЗАКОННОЕ ИХ СОЧЕТАНИЕ ──
   Разбор ОДИН на весь проект, и это сознательно иначе, чем у «длины».
   У «длины» разборов нарочно два: проба не смеет наследовать мягкость
   сервера, иначе позеленеет ровно там, где сервер ошибётся. У «границ»
   такого довода нет — их до наряда 26 не читал никто, — а два разбора
   дали бы серверу и пробе разные худшие точки, и describe разошёлся бы
   с договором молча. Разбор заякорен: незаякоренный вытащил бы «14»
   из хвоста «подпись рисуется при r больше 14».

   ХУДШИЕ СОСЕДИ — НИЖНИЕ. Слот тем теснее, чем меньше соседнее число
   (замер шага 0 наряда 26: полная сетка × 13 стилей; влияющих соседей
   три из восемнадцати, и у всех трёх предел растёт вместе с соседом).
   Оговорка одна и договорная: у нижнего края подписи может не быть
   в разметке вовсе — bub.bubbles.label рисуется только при r > 14.
   Объявлять длину тому, чего нет, нельзя, поэтому берётся худшая
   законная точка, В КОТОРОЙ ПОДПИСЬ ВООБЩЕ РИСУЕТСЯ: общий ход t
   от нижних краёв к верхним, шаг сотая, первая точка с живыми слотами.
   Это то же правило, по которому мёртвый слот не входит в замер,
   только применённое к диапазону соседа. */
const разборГраниц = сырое => {
  const m = /^(-?\d+(?:\.\d+)?)\s*[–—-]\s*(-?\d+(?:\.\d+)?)/.exec(String(сырое || ''));
  if (!m) return null;
  const низ = +m[1], верх = +m[2];
  return Number.isFinite(низ) && Number.isFinite(верх) && верх > низ ? { низ, верх } : null;
};

const худшиеСоседи = (отрисовка, поле, s, строк, границы) => {
  if (!Array.isArray(границы) || !границы.length) return null;
  for (let ш = 0; ш <= 100; ш++) {
    const наб = {};
    for (const г of границы) наб[г.путь] = +(г.низ + (ш / 100) * (г.верх - г.низ)).toFixed(4);
    const м = пробаПодполя(отрисовка, поле, 1, строк, наб);
    if (!м) return null;
    let svg; try { svg = отрисовка(s, м.данные, []); } catch { continue; }
    if (м.проба.some(р => доехала(svg, р))) return наб;
  }
  return null;
};

/* Числовые соседи подполя — из того же объявления массива, что и оно само.
   Отдаётся наружу, чтобы сервер и проба брали ОДИН список, а не собирали
   каждый свой: разойдись они — describe назвал бы одно число, договор
   другое, и поймать это стало бы нечем. */
const соседиМассива = правилоМассива => {
  const из = [];
  const спуск = (оп, путь) => {
    ...
    if (оп['тип'] !== 'число') return;
    const г = разборГраниц(оп['границы']);
    if (г) из.push({ путь: путь.join('.'), ...г });
  };
  for (const [к, з] of Object.entries((правилоМассива && правилоМассива['поля']) || {})) спуск(з, [к]);
  return из;
};
...
function elKpi(s,дан,зам){
  ...
  const [лбл,лfs]=ужать(label,bw,13,null,{сбор:зам,слот:своё(дан&&дан.label)?'label':null,s,зазор:s.ls});
  o+=txt(s,cx,base+22,t(s,лбл),лfs,s.wl,s.ts,s.ls);
  // value кладётся крупным кеглем и вылезает за карточку первым
  const [зн,зfs]=ужать(value,bw,38,null,{сбор:зам,слот:своё(дан&&дан.value)?'value':null});
  o+=txt(s,cx,base+76,зн,зfs,s.wv,s.mode==='neon'||s.mode==='retro'?s.ac:s.tp,-1);
  o+=chip(s,cx,base+92,ужать(delta,bч,12.5,12.5,{сбор:зам,слот:своё(дан&&дан.delta)?'delta':null})[0]);
  return wrapSvg(s,o);
}
...
function elBars(s,дан,зам){
  ...
  const [подписи,fs]=ужатьНабор(rows.map(р=>t(s,String(р.label==null?'':р.label))),bw-44,12,8.5,{сбор:зам,слоты:rows.map((_,i)=>вРяду(дан&&дан.rows,i,'label')?`rows[${i}].label`:null),s,зазор:s.ls});
  rows.forEach((р,i)=>{
...
function elColumn(s,дан,зам){
  ...
  const [згТ,згFs]=ужать(заг,pw,12,9,{сбор:зам,слот:своё(дан&&дан.title)?'title':null,s,зазор:s.ls});
  ...
  const [метки,мfs]=ужатьНабор(ряд.map(б=>t(s,String(б.label==null?'':б.label))),bw,11,7,{сбор:зам,слоты:ряд.map((_,i)=>вРяду(дан&&дан.bars,i,'label')?`bars[${i}].label`:null),s});
  ряд.forEach((б,i)=>{
...
function elDonut(s,дан,зам){
  ...
  const [ц,цfs]=ужать(центр,R*1.4,20,13,{сбор:зам,слот:своё(дан&&дан.center)?'center':null});
  ...
  const lx=cx+R+22;
  const легенда=доли.map((г,i)=>[String(г.label==null?'':г.label),пал[i%пал.length],i]).filter(п=>п[0]);
  const [имена,лfs]=ужатьНабор(легенда.map(п=>t(s,п[0])),x+w-20-(lx+15),11,8.5,{сбор:зам,слоты:легенда.map(п=>вРяду(дан&&дан.segments,п[2],'label')?`segments[${п[2]}].label`:null),s});
  легенда.forEach((п,i)=>{
...
function elRadar(s,дан,зам){
  ...
  const [згТ,згFs]=ужать(заг,w-40,12,9,{сбор:зам,слот:своё(дан&&дан.title)?'title':null,s,зазор:s.ls});
...
function elGauge(s,дан,зам){
  ...
  const [пдТ,пдFs]=ужать(подпись,w-40,10,null,{сбор:зам,слот:своё(дан&&дан.caption)?'caption':null,s});
...
function elArea(s,дан,зам){
  ...
  const [длТ,длFs]=ужать(delta,pw-8,12,8,{сбор:зам,слот:своё(дан&&дан.delta)?'delta':null});
  ...
  const [метки,мfs]=ужатьНабор(лбл.map(л=>t(s,л)),pw/Math.max(1,к)-6,9,6.5,{сбор:зам,слоты:лбл.map((_,i)=>вРяду(дан&&дан.labels,i)?`labels[${i}]`:null),s});
  лбл.forEach((л,i)=>
...
function elHeat(s,дан,зам){
  ...
  const [days,daysFs]=ужатьНабор(ряды(дан&&дан.cols,ОБР_heat.cols,нc).map(String),cw,9,7,{сбор:зам,слоты:ряды(дан&&дан.cols,ОБР_heat.cols,нc).map((_,i)=>вРяду(дан&&дан.cols,i)?`cols[${i}]`:null),s});
  ...
  const [wks,wksFs]=ужатьНаборБезТочек(ряды(дан&&дан.rows,ОБР_heat.rows,нr).map(String),ox-x-24,9,7,{сбор:зам,слоты:ряды(дан&&дан.rows,ОБР_heat.rows,нr).map((_,i)=>вРяду(дан&&дан.rows,i)?`rows[${i}]`:null)});
...
function elTree(s,дан,зам){
  ...
  B.forEach((b,i)=>{
    ...
    if(bw>52){
      const [блТ,блFs]=ужать(t(s,b.l),bw-14,9.5,7,{сбор:зам,слот:вРяду(дан&&дан.blocks,i,'label')?`blocks[${i}].label`:null,s});
      o+=txt(s,bx+9,by+17,блТ,блFs,s.wl,tc,null);
      const [бvТ,бvFs]=ужать(b.v,bw-14,i===0?15:11,8,{сбор:зам,слот:вРяду(дан&&дан.blocks,i,'v')?`blocks[${i}].v`:null});
...
function elScatter(s,дан,зам){
  ...
  const [осьТ,осьFs]=ужать(строка(дан&&дан.axis,ОБР_scat.axis),pw*.6,9,7,{сбор:зам,слот:своё(дан&&дан.axis)?'axis':null,s});
...
function elSpark(s,дан,зам){
  ...
  o+=txt(s,x+20,y+24,t(s,згТ),згFs,s.wl,s.ts,s.ls);
  const gl=s.glow?` filter="${s.glow}"`:'';
  const [ярл,яfs]=ужатьНабор(M.map(м=>t(s,м.l)),sx-lx-6,10.5,7,{сбор:зам,слоты:M.map((_,i)=>вРяду(дан&&дан.metrics,i,'label')?`metrics[${i}].label`:null),s});
  const [делты,дfs]=ужатьНабор(M.map(м=>м.v),rx-(sx+sw2)-6,10.5,7,{сбор:зам,слоты:M.map((_,i)=>вРяду(дан&&дан.metrics,i,'delta')?`metrics[${i}].delta`:null)});
  M.forEach((d,i)=>{
...
function elBubble(s,дан,зам){
  ...
  B.forEach(([l,X,Y,r,c],i)=>{
    ...
    if(r>14){const tc=nodeTx(s,c);
      const [бТ,бFs]=ужать(t(s,l),r*1.6,r>24?12:10,8,{сбор:зам,слот:вРяду(дан&&дан.bubbles,i,'label')?`bubbles[${i}].label`:null,s});
...
function elDumb(s,дан,зам){
  ...
  const [згТ,згFs]=ужать(заг,w-40,12,9,{сбор:зам,слот:своё(дан&&дан.title)?'title':null,s,зазор:s.ls});
  o+=txt(s,x+20,y+24,t(s,згТ),згFs,s.wl,s.ts,s.ls);
  const [подписи,мfs]=ужатьНабор(rows.map(р=>t(s,String(р.label==null?'':р.label))),tx-lx-6,10,7,{сбор:зам,слоты:rows.map((_,i)=>вРяду(дан&&дан.rows,i,'label')?`rows[${i}].label`:null),s});
  rows.forEach((р,i)=>{
    const ty=top+i*шаг;
...
function elArrows(s,дан,зам){
  ...
  const [имена,лfs]=ужатьНабор(L.map(п=>t(s,п[1])),aw-14,9.5,7,{сбор:зам,слоты:L.map((_,i)=>вРяду(дан&&дан.steps,i,'label')?`steps[${i}].label`:null),s});
  L.forEach(([n2,l,c],i)=>{
...
function elFunnel(s,дан,зам){
  ...
  const L=ур.map((у,i)=>[String(у.label==null?'':у.label),String(у.value==null?'':у.value),
    вРамках(зам,`levels[${i}].k`,Number(у.k)||0,.29,1),пал[i%пал.length]]);
  ...
  const gl=s.glow?` filter="${s.glow}"`:'';
  const [метки,мfs]=ужатьНабор(L.map(п=>t(s,п[0])),labR-x-18,9.5,7,{сбор:зам,слоты:L.map((_,i)=>вРяду(дан&&дан.levels,i,'label')?`levels[${i}].label`:null),s});
  L.forEach(([l,v,k,c],i)=>{
    ...
    const [vТ,vFs]=ужать(v,Math.max(nb,24),11,8,{сбор:зам,слот:вРяду(дан&&дан.levels,i,'value')?`levels[${i}].value`:null});
...
function elTimeline(s,дан,зам){
  ...
  const [пер,перFs]=ужатьНабор(P.map(п=>п[0]),шаг-6,11,8,{сбор:зам,слоты:P.map((_,i)=>вРяду(дан&&дан.points,i,'period')?`points[${i}].period`:null)});
  const [подп,пfs]=ужатьНабор(P.map(п=>t(s,п[1])),шаг-6,9.5,7,{сбор:зам,слоты:P.map((_,i)=>вРяду(дан&&дан.points,i,'label')?`points[${i}].label`:null),s});
...
function elCycle(s,дан,зам){
  ...
  const [ярл,яfs]=ужатьНабор(Q.map(п=>t(s,п[0])),46,9.5,7,{сбор:зам,слоты:Q.map((_,i)=>вРяду(дан&&дан.quadrants,i,'label')?`quadrants[${i}].label`:null),s});
  ...
  const [ц,цfs]=ужать(t(s,строка(дан&&дан.center,ОБР_cycle.center)),R*1.3,13,9,{сбор:зам,слот:своё(дан&&дан.center)?'center':null,s});
...
function elRoadmap(s,дан,зам){
  ...
  const R=ряды(дан&&дан.rows,ОБР_roadmap.rows,5)
    .map((р,i)=>[String(р.label==null?'':р.label),вРамках(зам,`rows[${i}].start`,Number(р.start)||0,0,1),
      вРамках(зам,`rows[${i}].len`,Number(р.len)||0,.27,1),пал[i%пал.length]]);
  ...
  cols.forEach((q,i)=>{
    ...
    const [qТ,qFs]=ужать(q,pw/cols.length-6,9,7,{сбор:зам,слот:вРяду(дан&&дан.cols,i)?`cols[${i}]`:null});
...
function elSwim(s,дан,зам){
  ...
  const [роли,рfs]=ужатьНабор(L.map(п=>t(s,п[0])),ox-x-12,9,6.5,{сбор:зам,слоты:L.map((_,i)=>вРяду(дан&&дан.lanes,i,'role')?`lanes[${i}].role`:null),s});
  L.forEach(([role,steps,c],i)=>{
    ...
    steps.forEach(([col,lab],j)=>{
      const sx=ox+col*pw/5;
      const [labТ,labFs]=ужать(lab,sw2+4,8.5,6.5,{сбор:зам,слот:вРяду(дан&&дан.lanes,i,'steps')?`lanes[${i}].steps[${j}].label`:null,s});
...
function elTree2(s,дан,зам){
  ...
  const [rootТ,rootFs]=ужать(строка(дан&&дан.root,ОБР_dtree.root),rw-8,9,7,{сбор:зам,слот:своё(дан&&дан.root)?'root':null,s});
  o+=node(s,cx-rw/2,yR,rw,nh,s.ac,rootТ,rootFs);
  const [метки,мfs]=ужатьНабор([B[0].node,B[1].node],cw-8,9,7,{сбор:зам,слоты:[0,1].map(k=>вРяду(дан&&дан.branches,k,'node')?`branches[${k}].node`:null),s});
  o+=node(s,cL-cw/2,yC,cw,nh,s.tm,метки[0],мfs);
  o+=node(s,cR-cw/2,yC,cw,nh,s.ac3,метки[1],мfs);
  if(внуки){
    const [лаб,лfs]=ужатьНабор(внуки,gw-8,8.5,6.5,{сбор:зам,слоты:внуки.map((_,k)=>вРяду(дан&&дан.branches,1,'children')?`branches[1].children[${k}]`:null),s});
    o+=node(s,gL-gw/2,yG,gw,nh,s.ac2,лаб[0],лfs);
    o+=node(s,gR-gw/2,yG,gw,nh,s.pos,лаб[1],лfs);
  }
  const [подп,пfs]=ужатьНабор([t(s,B[0].label),t(s,B[1].label)],44,8,6,{сбор:зам,слоты:[0,1].map(k=>вРяду(дан&&дан.branches,k,'label')?`branches[${k}].label`:null),s});
...
function elLoop(s,дан,зам){
  ...
  const [ярлN,яfsN]=ужатьНабор(N.map(п=>t(s,п[0])),30,8.5,6,{сбор:зам,слоты:N.map((_,i)=>вРяду(дан&&дан.nodes,i,'label')?`nodes[${i}].label`:null),s});
  N.forEach(([l,i])=>{
...
function elJourney(s,дан,зам){
  ...
  const [подп,пfs]=ужатьНабор(ST2.map(ст=>t(s,ст[0])),шаг-6,8.5,6.5,{сбор:зам,слоты:ST2.map((_,i)=>вРяду(дан&&дан.stages,i,'label')?`stages[${i}].label`:null),s});
...
function elTCards(s,дан,зам){
  ...
  o+=txt(s,x+20,y+24,t(s,згТ),згFs,s.wl,s.ts,s.ls);
  const gl=s.glow?` filter="${s.glow}"`:'';
  const [тексты,тfs]=ужатьНабор(C.map(п=>t(s,п[1])),cw-22,9,7,{сбор:зам,слоты:C.map((_,i)=>вРяду(дан&&дан.cards,i,'text')?`cards[${i}].text`:null),s});
  ...
  C.forEach(([d,l,c],i)=>{
    ...
    const [dТ,dFs]=ужать(d,60,9,7,{сбор:зам,слот:вРяду(дан&&дан.cards,i,'date')?`cards[${i}].date`:null});
...
function elPyramid(s,дан,зам){
  ...
  L.forEach(([l,c],i)=>{
    ...
    const [lТ,lFs]=ужать(t(s,l),maxW-16,9.5,7,{сбор:зам,слот:вРяду(дан&&дан.levels,i,'label')?`levels[${i}].label`:null,s});
...
function elSwot(s,дан,зам){
  ...
  const [ярлQ,яfsQ]=ужатьНабор(Q.map(п=>t(s,п[1])),qw-24,9,6.5,{сбор:зам,слоты:Q.map((_,i)=>вРяду(дан&&дан.quadrants,i,'label')?`quadrants[${i}].label`:null),s});
  const gl=s.glow?` filter="${s.glow}"`:'';
  Q.forEach(([ltr,lab,c,cxi,cyi],qi)=>{
    ...
    const [ltrТ,ltrFs]=ужать(ltr,qw-24,19,12,{сбор:зам,слот:вРяду(дан&&дан.quadrants,qi,'letter')?`quadrants[${qi}].letter`:null});
...
// 3 ── Матрица 2×2
/* Порядок подписей — канон Эйзенхауэра, разложенный по слотам договора
   (m22.композиция.порядок_слотов: [0] левый верхний, [1] правый верхний,
   [2] левый нижний, [3] правый нижний; ось X растёт вправо, Y — вверх):

     [0] важно, НЕ срочно      Спланировать
     [1] важно и срочно        Сделать          ← доминанта договора
     [2] НЕ важно, НЕ срочно   Убрать
     [3] НЕ важно, срочно      Делегировать

   До наряда 28 подписи стояли зеркально — все четыре не на местах, —
   и никакая сигнализация не срабатывала: разметка валидна, предупреждений
   нет, ошибка смысловая. Нашла её сторонняя проверка владельца 27 августа;
   договор порядка слотов тогда не описывал вовсе, и клиенту оставалось
   достроить его по этому образцу. Теперь описывает, и сторожит проба
   по координатам в tools/smoke.js. */
const ОБР_m22={title:'Приоритеты',axisX:'срочно →',axisY:'важно',
  quadrants:['Спланировать','Сделать','Убрать','Делегировать'].map(л=>({label:л}))};
function elM22(s,дан,зам){
  ...
  /* Четыре клетки закреплены за местами: порядок в данных задаёт смысл.
     МЕСТА (xM, yM) — порядок чтения, и они не менялись: их описывает
     договор. Переехали ЦВЕТА — вместе с подписями, слот в слот. Иначе
     вышло бы, что порядок починен, а обещание сломано: договор объявляет
     доминантой квадрант «сделать», доминанта стиля — s.pos, и, переставив
     одни подписи, мы увели бы акцент на чужой квадрант, а приглушённый
     s.tm достался бы не «Убрать». Пара «подпись ↔ цвет» сохранена
     поштучно: Сделать ↔ s.pos, Спланировать ↔ s.ac, Делегировать ↔ s.ac3,
     Убрать ↔ s.tm. Правка вариантом В, разрешение владельца — 13 пар,
     список в .superpowers/sdd/PLAN/task-28-pairs.md. */
  const цвM=[s.ac,s.pos,s.tm,s.ac3],xM=[0,1,0,1],yM=[0,0,1,1];
  ...
  const [ярлM,яfsM]=ужатьНабор(Q.map(п=>t(s,п[0])),pw/2-24,9,6.5,{сбор:зам,слоты:Q.map((_,i)=>вРяду(дан&&дан.quadrants,i,'label')?`quadrants[${i}].label`:null),s});
  ...
  const [axYТ,axYFs]=ужать(строка(дан&&дан.axisY,ОБР_m22.axisY),44,8.5,6.5,{сбор:зам,слот:своё(дан&&дан.axisY)?'axisY':null,s});
  o+=txt(s,x+22,oy+ph/2,t(s,axYТ),axYFs,s.wl,s.tm,null).replace('<text ','<text text-anchor="middle" ');
  const [axXТ,axXFs]=ужать(строка(дан&&дан.axisX,ОБР_m22.axisX),pw,8.5,6.5,{сбор:зам,слот:своё(дан&&дан.axisX)?'axisX':null,s});
...
function elVenn(s,дан,зам){
  ...
  const [leftТ,leftFs]=ужать(left,краяLR,9.5,7,{сбор:зам,слот:своё(дан&&дан.left)?'left':null,s});
  o+=txt(s,cx-d-16,cy+4,t(s,leftТ),leftFs,600,lab(s.ac),null).replace('<text ','<text text-anchor="middle" ');
  const [rightТ,rightFs]=ужать(right,краяLR,9.5,7,{сбор:зам,слот:своё(дан&&дан.right)?'right':null,s});
  ...
  const [стрЦ,цfs]=ужатьНабор(центр.map(с=>t(s,с)),2*r-d,8.5,7,{сбор:зам,слоты:центр.map((_,i)=>вРяду(дан&&дан.center,i)?`center[${i}]`:null),s});
...
function elFMatrix(s,дан,зам){
  ...
  const [згТ,згFs]=ужать(заг,w-40,12,9,{сбор:зам,слот:своё(дан&&дан.title)?'title':null,s,зазор:s.ls});
  o+=txt(s,x+20,y+24,t(s,згТ),згFs,s.wl,s.ts,s.ls);
  const [шапка,шFs]=ужатьНабор(P.map(п=>t(s,п)),colW,8.5,6.5,{сбор:зам,слоты:P.map((_,i)=>вРяду(дан&&дан.products,i)?`products[${i}]`:null),s});
  const [строки,сfs]=ужатьНабор(F.map(ф=>t(s,ф[0])),cx0-lx-colW/2-8,9,6.5,{сбор:зам,слоты:F.map((_,i)=>вРяду(дан&&дан.features,i,'label')?`features[${i}].label`:null),s});
...
function elNet(s,дан,зам){
  ...
  const [ярлL,яfsL]=ужатьНабор(обрL.map((об,i)=>t(s,String(сл[i]&&сл[i].label!=null?сл[i].label:об))),84,8.5,6.5,{сбор:зам,слоты:обрL.map((_,i)=>вРяду(дан&&дан.layers,i,'label')?`layers[${i}].label`:null),s});
  ярлL.forEach((l,i)=>
...
function elTrans(s,дан,зам){
  ...
  const [ярлR,яfsR]=ужатьНабор(R.map(п=>п[0]),bw-16,8.5,6.5,{сбор:зам,слоты:R.map((_,i)=>вРяду(дан&&дан.rows,i)?`rows[${i}]`:null),s});
  R.forEach(([l,c],i)=>{
  ...
  const [повТ,повFs]=ужать(строка(дан&&дан.repeats,ОБР_trans.repeats),прав-(brx+3),9,6.5,{сбор:зам,слот:своё(дан&&дан.repeats)?'repeats':null});
...
function elRag(s,дан,зам){
  ...
  const [ярлR,яfsR]=ужатьНабор(R.map(п=>п[0]),aw-6,9,6.5,{сбор:зам,слоты:R.map((_,i)=>вРяду(дан&&дан.steps,i,'label')?`steps[${i}].label`:null),s});
  R.forEach(([l,c],i)=>{
    const px=bx+i*(aw+11);
  ...
  const [храп,хfs]=ужать(строка(дан&&дан.store,ОБР_rag.store),vw-8,8.5,6.5,{сбор:зам,слот:своё(дан&&дан.store)?'store':null,s});
  ...
  const [ноТ,ноFs]=ужать(строка(дан&&дан.note,ОБР_rag.note),w-40,8,6.5,{сбор:зам,слот:своё(дан&&дан.note)?'note':null,s});
...
function elLLM(s,дан,зам){
  ...
  const [имена,иfs]=ужатьНабор(M.map(м=>t(s,м[0])),px-lx-8,10,7,{сбор:зам,слоты:M.map((_,i)=>вРяду(дан&&дан.models,i,'name')?`models[${i}].name`:null),s});
  const [пары,пfs]=ужатьНабор(M.map(м=>м[1]),px-lx-8,8,6.5,{сбор:зам,слоты:M.map((_,i)=>вРяду(дан&&дан.models,i,'params')?`models[${i}].params`:null)});
...
function elVec(s,дан,зам){
  ...
  CL.forEach((cl,i)=>{
    ...
    if(i<2){
      ...
      const [клТ,клFs]=ужать(t(s,l),budget,9,7,{сбор:зам,слот:своё(скопДан[i])?`clusters[${i}]`:null,s});
  ...
  const [примТ,примFs]=ужать(прим,w-40,8,6.5,{сбор:зам,слот:своё(дан&&дан.note)?'note':null,s});
...
function elMicro(s,дан,зам){
  ...
  const [ярлS,яfsS]=ужатьНабор(S.map(п=>п[0]),sw2-6,8.5,6.5,{сбор:зам,слоты:S.map((_,i)=>вРяду(дан&&дан.services,i,'label')?`services[${i}].label`:null),s});
  const dy=sy+36;
  S.forEach((_,i)=>{const sx=x+26+i*(sw2+9);
    o+=conn(s,gx+gw/2,gy+bh,sx+sw2/2,sy);});
  const [шлз,шfs]=ужать(строка(дан&&дан.gateway,ОБР_micro.gateway),gw-10,9,7,{сбор:зам,слот:своё(дан&&дан.gateway)?'gateway':null,s});
...
function elCicd(s,дан,зам){
  ...
  const [ярлP,яfsP]=ужатьНабор(P.map(п=>t(s,п[0])),bw-4,7.5,6,{сбор:зам,слоты:P.map((_,i)=>вРяду(дан&&дан.stages,i,'label')?`stages[${i}].label`:null),s});
  ...
  const [примТ,примFs]=ужать(прим,pw,8.5,7,{сбор:зам,слот:своё(дан&&дан.note)?'note':null,s});
...
function elEgov(s,дан,зам){
  ...
  const [акт,аfs]=ужать(строка(дан&&дан.actor,ОБР_egov.actor),cw-8,8.5,6.5,{сбор:зам,слот:своё(дан&&дан.actor)?'actor':null,s});
  o+=node(s,cx-cw/2,top,cw,bh,s.ac3,акт,аfs);
  o+=`<line x1="${cx}" y1="${top+bh}" x2="${cx}" y2="${top+bh+12}" stroke="${s.ln}" stroke-width="1.4"/>`;
  const hy=top+bh+12;
  const [шлюз,шfs]=ужать(строка(дан&&дан.hub,ОБР_egov.hub),w-70,9,7,{сбор:зам,слот:своё(дан&&дан.hub)?'hub':null,s});
  ...
  const [ярлM2,яfsM2]=ужатьНабор(M.map(п=>п[0]),mw-6,8.5,6.5,{сбор:зам,слоты:M.map((_,i)=>вРяду(дан&&дан.agencies,i,'label')?`agencies[${i}].label`:null),s});
  ...
  const [примТ,примFs]=ужать(прим,w-40,7.5,6.5,{сбор:зам,слот:своё(дан&&дан.note)?'note':null,s});
  o+=txt(s,x+20,y+h-18,t(s,примТ),примFs,s.wl,s.tm,null);
  return wrapSvg(s,o);
}
...
function elBus(s,дан,зам){
  ...
  const [ярлPR,яfsPR]=ужатьНабор(PR.map(п=>п[0]),bw-6,8,6,{сбор:зам,слоты:PR.map((_,i)=>вРяду(дан&&дан.producers,i,'label')?`producers[${i}].label`:null),s});
  const [ярлCO,яfsCO]=ужатьНабор(CO.map(п=>п[0]),bw-6,8,6,{сбор:зам,слоты:CO.map((_,i)=>вРяду(дан&&дан.consumers,i,'label')?`consumers[${i}].label`:null),s});
  ...
  const [шина,шfs]=ужать(строка(дан&&дан.bus,ОБР_bus.bus),w-70,9,7,{сбор:зам,слот:своё(дан&&дан.bus)?'bus':null,s});
...
function elLayers(s,дан,зам){
  ...
  const [примТ,примFs]=ужать(прим,bw,7.5,6.5,{сбор:зам,слот:своё(дан&&дан.note)?'note':null,s});
  ...
  const [имена,иfs]=ужатьНабор(L.map(п=>t(s,п[0])),bw*.45,9.5,7,{сбор:зам,слоты:L.map((_,i)=>вРяду(дан&&дан.layers,i,'label')?`layers[${i}].label`:null),s});
  const [стеки,сfs]=ужатьНабор(L.map(п=>t(s,п[1])),bw*.52,8,6,{сбор:зам,слоты:L.map((_,i)=>вРяду(дан&&дан.layers,i,'tech')?`layers[${i}].tech`:null),s});
  L.forEach(([l,sub,c],i)=>{
    const by=top+i*(bh+g);
...
function elBigNum(s,дан,зам){
  ...
  const [eyebrowТ,eyebrowFs]=ужать(eyebrow,maxW,9.5,null,{сбор:зам,слот:своё(дан&&дан.eyebrow)?'eyebrow':null,s,зазор:2.2});
  o+=txt(s,cx,y+34,t(s,eyebrowТ),eyebrowFs,s.wl,s.tm,2.2).replace('<text ','<text text-anchor="middle" ');
  const [valueТ,valueFs]=ужать(value,maxW,52,null,{сбор:зам,слот:своё(дан&&дан.value)?'value':null});
  ...
  const [captionТ,captionFs]=ужать(caption,maxW,11,null,{сбор:зам,слот:своё(дан&&дан.caption)?'caption':null,s});
  o+=txt(s,cx,y+120,t(s,captionТ),captionFs,s.wl,s.ts,null).replace('<text ','<text text-anchor="middle" ');
  const [deltaТ,deltaFs]=ужать(delta,maxW,9.5,null,{сбор:зам,слот:своё(дан&&дан.delta)?'delta':null,s});
...
function elFacts(s,дан,зам){
  ...
  const [подписи,пfs]=ужатьНабор(F.map(ф=>t(s,ф[1])),cw-16,9,6.5,{сбор:зам,слоты:F.map((_,i)=>вРяду(дан&&дан.facts,i,'label')?`facts[${i}].label`:null),s});
  F.forEach(([v,l,c],i)=>{
    ...
    const [зн,зfs]=ужать(v,bw-10,v.length>4?17:24,11,{сбор:зам,слот:вРяду(дан&&дан.facts,i,'value')?`facts[${i}].value`:null});
...
function elStatCard(s,дан,зам){
  ...
  const [labelТ,labelFs]=ужать(label,w-48,9.5,null,{сбор:зам,слот:своё(дан&&дан.label)?'label':null,s,зазор:1.8});
  o+=txt(s,lx,y+34,t(s,labelТ),labelFs,s.wl,s.tm,1.8);
  const [valueТ,valueFs]=ужать(value,maxWv,38,null,{сбор:зам,слот:своё(дан&&дан.value)?'value':null});
  ...
  const [deltaТ,deltaFs]=ужать(delta,w-48,10,null,{сбор:зам,слот:своё(дан&&дан.delta)?'delta':null,s});
  ...
  const [noteТ,noteFs]=ужать(note,w-48,9,null,{сбор:зам,слот:своё(дан&&дан.note)?'note':null,s});
...
function elRing(s,дан,зам){
  ...
  const [имена,иfs]=ужатьНабор(лег.map(л=>t(s,л[0])),avail,9.5,7,{сбор:зам,слоты:лег.map((_,i)=>вРяду(дан&&дан.legend,i,'label')?`legend[${i}].label`:null),s});
  const [субт,сfs]=ужатьНабор(лег.map(л=>t(s,л[1])),avail,8.5,6.5,{сбор:зам,слоты:лег.map((_,i)=>вРяду(дан&&дан.legend,i,'sub')?`legend[${i}].sub`:null),s});
  лег.forEach(([,,c],i)=>{
...
function elIcons(s,дан,зам){
  ...
  const [подписи,пfs]=ужатьНабор(I.map(э=>t(s,э[1])),cw-10,9,6.5,{сбор:зам,слоты:I.map((_,i)=>вРяду(дан&&дан.items,i,'label')?`items[${i}].label`:null),s});
  I.forEach(([v,l,c],i)=>{
    ...
    const [vТ,vFs]=ужать(v,r*1.85,11,8,{сбор:зам,слот:вРяду(дан&&дан.items,i,'value')?`items[${i}].value`:null});
  ...
  const [примТ,примFs]=ужать(прим,w-40,8.5,7,{сбор:зам,слот:своё(дан&&дан.note)?'note':null,s});
  o+=txt(s,x+20,y+h-18,t(s,примТ),примFs,s.wl,s.tm,null);
  return wrapSvg(s,o);
}
...
function elQuote(s,дан,зам){
  ...
  const [строкиЦ,цfs]=ужатьНабор(цит.map(с=>t(s,с)),x+w-22-(lx+24),11.5,8.5,{сбор:зам,слоты:цит.map((_,i)=>вРяду(дан&&дан.quote,i)?`quote[${i}]`:null),s});
  строкиЦ.forEach((с,i)=>o+=txt(s,lx+24,y+34+i*16,с,цfs,s.wl,s.tp,null));
  const авт=строка(дан&&дан.author,ОБР_quote.author);
  const [автТ,автFs]=ужать(авт,x+w-22-(lx+24),9,7,{сбор:зам,слот:своё(дан&&дан.author)?'author':null,s});
  ...
  const [подписиЦ,пfsЦ]=ужатьНабор(S.map(э=>t(s,э[1])),cw-8,8.5,6.5,{сбор:зам,слоты:S.map((_,i)=>вРяду(дан&&дан.stats,i,'label')?`stats[${i}].label`:null),s});
  S.forEach(([v,l,c],i)=>{
    const ccx=lx+cw*i+cw/2-4;
    const [vТ,vFs]=ужать(v,cw-8,20,13,{сбор:зам,слот:вРяду(дан&&дан.stats,i,'value')?`stats[${i}].value`:null});
...
function elBefore(s,дан,зам){
  ...
  const cols=[['было',s.tm,было,0,'before'],['стало',s.ac,стало,1,'after']];
  // обе колонки берут единый кегль: разный размер читался бы как разная важность.
  /* Голоса у этого места нарочно нет, и это не забытый слот. Набор здесь
     считает ТОЛЬКО кегль — сам список отброшен (const [,сfs]), а рисуется
     то, что ниже ужимает каждую строку заново тем же кеглем и в ту же
     ширину. Скажи оба — на каждый before[i] и after[i] пришло бы по две
     записи с одинаковым текстом, и модель решила бы, что беды две.
     Говорит то место, чей результат попадает в разметку. */
  const [,сfs]=ужатьНабор([...было,...стало].map(в=>t(s,в)),cw-12,11,7.5,{s});
  cols.forEach(([hd,c,rows,hi,поле])=>{
    ...
    rows.forEach((v,ri)=>o+=txt(s,bx+cw/2,r0+ri*rs,ужать(t(s,v),cw-12,сfs,сfs,{сбор:зам,слот:вРяду(дан&&дан[поле],ri)?`${поле}[${ri}]`:null,s})[0],сfs,hi?700:s.wl,tc,null).replace('<text ','<text text-anchor="middle" '));
  });
...
function elDivider(s,дан,зам){
  ...
  const [eyebrowТ,eyebrowFs]=ужать(eyebrow,maxW,9,null,{сбор:зам,слот:своё(дан&&дан.eyebrow)?'eyebrow':null,s,зазор:3});
  ...
  const [numТ,numFs]=ужать(num,maxW,44,null,{сбор:зам,слот:своё(дан&&дан.num)?'num':null});
  o+=txt(s,cx,y+80,numТ,numFs,s.wv,ac,-1.5).replace('<text ','<text text-anchor="middle" ');
  const [згТ,згFs]=ужать(заг,maxW,18,null,{сбор:зам,слот:своё(дан&&дан.title)?'title':null,s});
  ...
  const [subТ,subFs]=ужать(sub,maxW,9.5,null,{сбор:зам,слот:своё(дан&&дан.sub)?'sub':null,s});
...
function elChapter(s,дан,зам){
  ...
  const [eyebrowТ,eyebrowFs]=ужать(eyebrow,maxW,9,null,{сбор:зам,слот:своё(дан&&дан.eyebrow)?'eyebrow':null,s,зазор:2.6});
  ...
  const [стрГ,гfs]=ужатьНабор(строки.map(с=>t(s,с)),maxW,10.5,7,{сбор:зам,слоты:строки.map((_,i)=>вРяду(дан&&дан.lines,i)?`lines[${i}]`:null),s});
  ...
  const [metaТ,metaFs]=ужать(meta,maxW,9,null,{сбор:зам,слот:своё(дан&&дан.meta)?'meta':null,s});
...
function elThanks(s,дан,зам){
  ...
  const [згТ,згFs]=ужать(заг,maxW,34,null,{сбор:зам,слот:своё(дан&&дан.title)?'title':null,s});
  ...
  const [имяТ,имяFs]=ужать(имя,maxW,12,null,{сбор:зам,слот:своё(дан&&дан.name)?'name':null,s});
  o+=txt(s,cx,y+100,t(s,имяТ),имяFs,600,s.ts,null).replace('<text ','<text text-anchor="middle" ');
  const [оргТ,оргFs]=ужать(орг,maxW,9.5,null,{сбор:зам,слот:своё(дан&&дан.org)?'org':null,s});
  ...
  o+=txt(s,cx,cy2-1,ужать(контакт,bw2-13*2,9.5,9.5,{сбор:зам,слот:своё(дан&&дан.contact)?'contact':null})[0],9.5,600,cc,null).replace('<text ','<text text-anchor="middle" ');
  return wrapSvg(s,o);
}
...
function elAgenda(s,дан,зам){
  ...
  const [згТ,згFs]=ужать(заг,w-44,13,10,{сбор:зам,слот:своё(дан&&дан.title)?'title':null,s});
  ...
  const [пп,пfs]=ужатьНабор(A.map(п=>t(s,п[1])),x+w-22-(x+46),10.5,8,{сбор:зам,слоты:A.map((_,i)=>вРяду(дан&&дан.items,i)?`items[${i}]`:null),s});
  A.forEach(([n2,l,c],i)=>{
    const ly=y+56+i*21;
...
function elSpeaker(s,дан,зам){
  ...
  const полнИн=строка(дан&&дан.initials,ОБР_speaker.initials);
  const initials=полнИн.slice(0,2);
  /* Единственная обрезка в библиотеке, которую делает не ужать(): кружок держит
     ровно два знака, кегль здесь не ужимается принципиально (см. выше), и потому
     через подгонку эта строка не проходит вовсе. Молчала она от этого ничуть
     не меньше — третий знак пропадал, и узнать об этом было неоткуда. Проба
     наряда 29 спрашивает голос с КАЖДОГО поля договора, а не с каждого места
     подгонки, и на этом поле поймала библиотеку одну на тринадцать стилей.
     Ширины настоящие: та же мера библиотеки и тот же кегль 19, что в разметке;
     t() здесь не зовётся, поэтому и мера идёт без стиля, без капса. */
  if(полнИн.length>2&&своё(дан&&дан.initials))
    переполнение({сбор:зам,слот:'initials'},полнИн,мера(полнИн,19),мера(initials,19),'обрезано до двух знаков');
  ...
  const [имяТ,имяFs]=ужать(имя,maxW,14,null,{сбор:зам,слот:своё(дан&&дан.name)?'name':null,s});
  o+=txt(s,lx,y+50,t(s,имяТ),имяFs,s.wv,s.mode==='neon'||s.mode==='retro'?s.ac:s.tp,-.2);
  const [рольТ,рольFs]=ужать(роль,maxW,10,null,{сбор:зам,слот:своё(дан&&дан.role)?'role':null,s});
  ...
  const [стр1Т,стр1Fs]=ужать(стр1,maxW,9,null,{сбор:зам,слот:своё(дан&&дан.line1)?'line1':null,s});
  o+=txt(s,lx,y+118,t(s,стр1Т),стр1Fs,s.wl,s.ts,null);
  const [стр2Т,стр2Fs]=ужать(стр2,maxW,9,null,{сбор:зам,слот:своё(дан&&дан.line2)?'line2':null,s});
...
function elFaq(s,дан,зам){
  ...
  const [згТ,згFs]=ужать(заг,w-44,12,9,{сбор:зам,слот:своё(дан&&дан.title)?'title':null,s,зазор:s.ls});
  o+=txt(s,x+22,y+28,t(s,згТ),згFs,s.wl,s.ts,s.ls);
  const [вопр,вfs]=ужатьНабор(Q.map(п=>t(s,п[0])),bw-40,9.5,7,{сбор:зам,слоты:Q.map((_,i)=>вРяду(дан&&дан.items,i,'q')?`items[${i}].q`:null),s});
  let cy=top;
  Q.forEach(([q,open,ans],i)=>{
    ...
    if(open){const [ansТ,ansFs]=ужать(ans,bw-28,8.5,7,{сбор:зам,слот:вРяду(дан&&дан.items,i,'a')?`items[${i}].a`:null,s});o+=txt(s,bx+14,cy+32,t(s,ansТ),ansFs,s.wl,s.tm,null);}
    cy+=bh+6;
...
/* «мера» отдана наружу не для пользы отрисовки, а под пробу: закон
   «мера знает про моноширинность» проверяется только на самой мере, и
   проверять его через отрисовку значило бы гадать. Внутри библиотеки
   ничего не меняется — функция та же, вызовы те же. */
export { CATS, ВЕРХ_ПРЕДЕЛА, ПРОБА_ПРЕДЕЛА, мера, наборПробы, пробаПодполя, пределЗнаков, разборГраниц, худшиеСоседи, соседиМассива, словами };
